...
void ObjectMgr::LoadCreatureTemplates()
{
    ...
    m_CreatureTemplateStore     = new CreatureTemplate*[m_CreatureTemplateStoreSize+1];
    ...
    do
    {
        ...
        creatureTemplate->RaidData = sObjectMgr->GetRaidEncounterData(entry);

        m_CreatureTemplateStore[entry] = creatureTemplate;
        ++count;
    }
    while (result->NextRow());
    ...
    do
    {
        ...
        switch (linkType)
        {
            case CREATURE_TO_CREATURE:
            {
                ...
                guid = MAKE_NEW_GUID(guidLow, slave->id, HighGuid::Unit);
                linkedGuid = MAKE_NEW_GUID(linkedGuidLow, master->id, HighGuid::Unit);
                break;
            }
            case CREATURE_TO_GO:
            {
...
bool ObjectMgr::SetCreatureLinkedRespawn(uint32 guidLow, uint32 linkedGuidLow)
{
    ...
    uint64 guid = MAKE_NEW_GUID(guidLow, master->id, HighGuid::Unit);
    ...
    uint64 linkedGuid = MAKE_NEW_GUID(linkedGuidLow, slave->id, HighGuid::Unit);
...
{
    uint64 guid = MAKE_NEW_GUID(lowguid, 0, HighGuid::Player);
...
uint64 ObjectMgr::GetPlayerGUIDByName(std::string name) const
{
    ...
    if (result)
        guid = MAKE_NEW_GUID((*result)[0].GetUInt32(), 0, HighGuid::Player);
...
void ObjectMgr::LoadRealmCompletedChallenges()
{
    ...
    do
    {
        ...
        {
            l_RealmChallenge.m_Members[l_I].m_Guid = MAKE_NEW_GUID(l_Fields[l_Index++].GetUInt32(), 0, HighGuid::Player);
...
void ObjectMgr::LoadChallengeRewards()
{
    ...
    QueryResult l_Result = WorldDatabase.Query("SELECT map_id, none_money, bronze_money, silver_money, gold_money, none_valor, bronze_valor, silver_valor, gold_valor, title, gold_achievement FROM challenge_mode_rewards");
    if (!l_Result)
    {
    ...
    do
    {
        ...
        for (uint8 l_I = 0; l_I < 4; ++l_I)
            l_Rewards.ValorReward[l_I] = l_Fields[l_Index++].GetUInt32();
...
void ObjectMgr::LoadItemTemplates()
{
    ...
    {
        if (DisableMgr::IsDisabledFor(DISABLE_TYPE_ITEM, itemId, nullptr))
            continue;
        ...
        /// HACK FIX ALERT - Notes on Lightning Steel
        if (itemId == 94553)
            itemTemplate.BuyPrice = 500000000;
        ...
        for (uint32 i = 0; i < MAX_ITEM_PROTO_STATS; ++i)
        {
            ...
            itemTemplate.ItemStat[i].ItemScalingValue = sparse->ItemScalingValue[i];
            itemTemplate.ItemStat[i].ItemSocketCostRate = sparse->ItemSocketCostRate[i];
            
            if ((itemId == 25285 || itemId == 36659) && sparse->ItemStatType[i] == ITEM_MOD_SPELL_POWER)
                itemTemplate.ItemStat[i].ItemStatValue = 7;
        }
    ...
    if (result)
    {
        do
        {
            ...
            bool exist = _itemTemplateStore.find(itemId) != _itemTemplateStore.end();

            ItemTemplate& itemTemplate = _itemTemplateStore[itemId];
            itemTemplate.RequiredLevel = uint32(fields[18].GetUInt8());

            auto name = fields[4].GetString();
            if (name.length())
            {
                ItemLocale& data = _itemLocaleStore[itemId];
                AddLocaleString(name, LOCALE_enUS, data.Name);
            }
            ...
            for (uint32 i = 0; i < MAX_ITEM_PROTO_STATS; ++i)
            {
                ...
                itemTemplate.ItemStat[i].ItemScalingValue  = fields[29 + i * 4 + 2].GetInt32();
                itemTemplate.ItemStat[i].ItemSocketCostRate = fields[29 + i * 4 + 3].GetInt32();
            }
...
void ObjectMgr::LoadAreaTriggerTemplates()
{
    ...
    QueryResult l_Result = WorldDatabase.Query("SELECT `spell_id`, `eff_index`, `entry`, `type`, `scale_x`, `scale_y`, `flags`, `move_curve_id`, `scale_curve_id`, `morph_curve_id`,"
        ...
        "`facing_curve_id`, `CheckRadius`, `SelectionMode`, `ScriptName`, `data0`, `data1`, `data2`, `data3`, `data4`, `data5`, `data6`, `data7` FROM `areatrigger_template`");
    if (!l_Result)
    {
    ...
    do
    {
        ...
        memset(&l_Template, 0, sizeof(AreaTriggerTemplate));
        ...
        l_Template.m_CheckRadius   = l_Fields[l_Index++].GetFloat();
        l_Template.m_SelectionMode = l_Fields[l_Index++].GetUInt8();
    ...
    do
    {
        ...
        switch (tmp.command)
        {
            ...
            case SCRIPT_COMMAND_TEMP_SUMMON_CREATURE:
            {
                ...
                if (tmp.TempSummonCreature.Unique >= 2)
                {
                    sLog->outError(LOG_FILTER_SQL, "Table `%s` has invalid dataint value (Unique: %u) in SCRIPT_COMMAND_TEMP_SUMMON_CREATURE for script id %u",
                        tableName.c_str(), tmp.TempSummonCreature.Unique, tmp.id);
                    continue;
                }
                break;
            }
            case SCRIPT_COMMAND_DESPAWN_CREATURE:
            {
                ...
                {
                    sLog->outError(LOG_FILTER_SQL, "Table `%s` has invalid creature (Entry: %u) in SCRIPT_COMMAND_DESPAWN_CREATURE for script id %u",
...
void ObjectMgr::LoadSpellAreaTriggers()
{
    uint32 oldMSTime = getMSTime();

    _spellAreaTriggerStore.clear();                        
    QueryResult result = WorldDatabase.Query("SELECT spell_id, selection, visualRadius, radius, ScriptName FROM spell_areatrigger");

    if (!result)
    {
        sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 spell areatriggers. DB table `spell_areatrigger` is empty.");
        return;
    }
    ...
    do
    {
        ...
        uint8 selection = fields[1].GetUInt32();
        float visualRadius = fields[2].GetFloat();
        float radius = fields[3].GetFloat();
        const char *scriptName = fields[4].GetCString();


        SpellAreaTrigger& trigger = _spellAreaTriggerStore[spellId];

        trigger.spell_id = spellId;
        trigger.selection = SpellAreaTriggerSelection(selection);
        trigger.visualRadius = visualRadius;
        trigger.radius = radius;
        trigger.script_id = GetScriptId(scriptName);

    } while (result->NextRow());

    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded %u spell areatrigger scripts in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}

SpellAreaTrigger const* ObjectMgr::GetSpellAreaTrigger(uint32 spellId) const
{
    SpellAreaTriggerContainer::const_iterator itr = _spellAreaTriggerStore.find(spellId);
    if (itr != _spellAreaTriggerStore.end())
        return &(itr->second);
...
uint32 ObjectMgr::GenerateCreatureSpawnId()
{
    ...
    {
        sLog->outError(LOG_FILTER_GENERAL, "Creature spawn id overflow!! Can't continue, shutting down server. ");
        World::StopNow(ERROR_EXIT_CODE);
    }
...
void ObjectMgr::LoadNPCSpellClickSpells()
{
    ...
    for (uint32 l_Entry = 0; l_Entry < l_LastEntry; l_Entry++)
    {
        ...
        if ((l_CreatureTemplate->npcflag & UNIT_NPC_FLAG_SPELLCLICK) && _spellClickInfoStore.find(l_Entry) == _spellClickInfoStore.end())
        {
            ...
            const_cast<CreatureTemplate*>(l_CreatureTemplate)->npcflag &= ~UNIT_NPC_FLAG_SPELLCLICK;
        }
    }
...
uint32 ObjectMgr::GetSpellAreaTriggerScriptId(uint32 trigger_id)
{
    SpellAreaTriggerContainer::const_iterator i = _spellAreaTriggerStore.find(trigger_id);
    if (i != _spellAreaTriggerStore.end())
        return i->second.script_id;
    return 0;
}
...
void ObjectMgr::LoadScriptNames()
{
    ...
    QueryResult result = WorldDatabase.Query(
      ...
      "UNION "
      "SELECT DISTINCT(ScriptName) FROM spell_areatrigger WHERE ScriptName <> '' "
      "UNION "
...
void ObjectMgr::LoadFactionChangeQuests()
{
    ...
    QueryResult result = WorldDatabase.Query("SELECT allianceQuestId, hordeQuestId FROM player_factionchange_quests");
    ...
    do
    {
        ...
        uint32 allianceQuest = fields[0].GetUInt32();
        uint32 hordeQuest = fields[1].GetUInt32();

        if (!sObjectMgr->GetQuestTemplate(allianceQuest))
        {
            sLog->outError(LOG_FILTER_SQL, "Alliance quest %u referenced in `player_factionchange_quests` does not exist, pair skipped!", allianceQuest);
            continue;
        }

        if (!sObjectMgr->GetQuestTemplate(hordeQuest))
        {
            sLog->outError(LOG_FILTER_SQL, "Horde quest %u referenced in `player_factionchange_quests` does not exist, pair skipped!", hordeQuest);
            continue;
        }

        FactionChange_Quests[allianceQuest] = hordeQuest;
...
{
    if (entry >= m_CreatureTemplateStoreSize)
        return nullptr;

    return m_CreatureTemplateStore[entry];
}
...
{
    std::map<uint32, std::vector<QuestPOIPointEntry const*>> poiList;
    for (uint32 i = 0; i < sQuestPOIPointStore.GetNumRows(); ++i)
    {
        ...
        poiList[poi->POIId].push_back(poi);
    }

    std::map<uint32, uint32> areaIdToZone;
    for (uint32 i = 0; i < 6853; ++i) // hardcore
    {
        if (WorldMapAreaEntry const* areaEntry = sWorldMapAreaStore.LookupEntry(i))
        {
            areaIdToZone[areaEntry->ID] = areaEntry->area_id;
        }
    }

    // hack fix due to dublicate values
    areaIdToZone[141] = 15;
    areaIdToZone[606] = 616;
    areaIdToZone[700] = 4922;
    areaIdToZone[720] = 5034;
    areaIdToZone[857] = 6134;
        
    for (ResearchSiteEntry const* site : sResearchSiteSet)
    {
        auto itr = poiList.find(site->POIid);
        if (itr == poiList.end())
            continue;

        QuestPOIBlobEntry const* blob = sQuestPOIBlobStore.LookupEntry(site->POIid);
        if (!blob)
            continue;

        ResearchZoneEntry& zone = _researchZoneMap[site->ID];
        for (QuestPOIPointEntry const* point : itr->second)
        {
            zone.coords.push_back(ResearchPOIPoint(point->X, point->Y));
        }

        zone.map = blob->MapId;
        zone.zone = areaIdToZone[blob->WorldMapAreaID];
        zone.level = 0;
        for (uint32 i = 0; i < sAreaStore.GetNumRows(); ++i)
        {
            ...
            if (area->mapid == zone.map && area->zone == zone.zone)
            {
                zone.level = area->area_level;
                break;
            }
        }
    }
...
void ObjectMgr::LoadItemUpgradeHotfix()
{
    QueryResult result = WorldDatabase.PQuery("SELECT Id, itemUpgradePath, itemLevelUpgrade, prevItemUpgradeId, currencyId, currencyCost FROM item_upgrade_hotfix");
    if (!result)
    {
        sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 item upgrade hotfix info. DB table `item_upgrade_hotfix` is empty.");
        return;
    }
    ...
    do
    {
        ...
        ItemUpgradeEntry* proto = new ItemUpgradeEntry();

        proto->Id = field[index++].GetUInt32();
        proto->itemUpgradePath = field[index++].GetUInt32();
        proto->itemLevelUpgrade = field[index++].GetUInt32();
        proto->precItemUpgradeId = field[index++].GetUInt32();
        proto->currencyId = field[index++].GetUInt32();
        proto->currencyCost = field[index++].GetUInt32();

        sItemUpgradeStore.EraseEntry(proto->Id);
        sItemUpgradeStore.AddEntry(proto->Id, (const ItemUpgradeEntry*)proto);

        ++counter;
    }
    while (result->NextRow());

    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded %u item upgrade hotfix info.", counter);
}
...
void ObjectMgr::LoadRaidEncounterData()
{
    uint32 oldMSTime = getMSTime();

    QueryResult result = WorldDatabase.Query("SELECT entry, encounterId, encounterMask FROM `raid_encounters`");
    if (!result)
    {
        sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 Raid Encounters. DB table `raid_encounters` is empty.");
        return;
    }
    ...
    do
    {
        ...
        uint32 entry = fields[0].GetUInt32();
        uint32 encounterId = fields[1].GetUInt32();
        uint32 encounterMask = fields[2].GetUInt32();

        RaidEncounterData& data = m_RaidEncounterData[entry];
        data.entry = entry;
        data.encounterId = encounterId;
        data.encounterMask = encounterMask;

        count++;
    } while (result->NextRow());

    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded %u Raid Encounters in %u ms.", count, GetMSTimeDiffToNow(oldMSTime));

}

RaidEncounterData const* ObjectMgr::GetRaidEncounterData(uint32 entry) const
{
    RaidEncounterDataMap::const_iterator itr = m_RaidEncounterData.find(entry);
    if (itr == m_RaidEncounterData.end())
        return nullptr;

    return &itr->second;
}

RaidEncounterData* ObjectMgr::GetRaidEncounterData(uint32 entry)
{
    RaidEncounterDataMap::iterator itr = m_RaidEncounterData.find(entry);
    if (itr == m_RaidEncounterData.end())
        return nullptr;
