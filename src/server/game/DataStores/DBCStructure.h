...
struct CriteriaValueEntry
{
    ...
    CriteriaValueEntry(uint32 _asset, uint32 _amount)
    {
        raw.asset = _asset;
        raw.amount = _amount;
    }
...
struct FactionEntry
{
    ...
    bool IsExpansionFit(uint32 expansion) const
    {
        uint32 mask = 0;
        switch (team)
        {
...
struct ScalingStatValuesEntry
{
    ...
    uint32 ShieldArmor;                                     // 49       Armor for shield
...
struct SkillLineEntry
{
    ...
    uint32 parentSkillLineID;                               // 7    Only cooking skill id
...
struct SkillLineAbilityEntry
{
    ...
    uint32    NumSkillUps;                                  // 10       m_skillgain 1-30
    uint32    UniqueBit;                                    // 11       4.0.0 unk (increment id)
    uint32    TradeSkillCategoryID;                         // 12       5.0.5
};
...
struct SpellEffectEntry
{
    ...
    float     EffectSpellPowerBonus;                        // 7         m_effectBonus
...
struct SpellPowerEntry
{
    ...
    //uint32    unk_4;                                      // 7
    //uint32    unk_5;                                      // 8
    float       CostBasePercentage;                         // 9
    float       CostPerSecondPercentage;                    // 10
    uint32      RequiredAuraSpellId;                        // 11 Shapeshift required (spellID)
    float       CostMaxPercentage;                          // 12
};
...
struct SpellTargetRestrictionsEntry
{
    ...
    uint32  SpellId;                                        // 1 m_spellId
    //uint32    Unknown;                                    // 2
    float   SelectionDegrees;                               // 3 m_maxTargetRadius
...
struct SpellItemEnchantmentEntry
{
    ...
    uint32      MinLevel;                                   // 21       m_minLvl
    uint32      MaxLevel;                                   // 22       m_maxLvl
...
struct UnitPowerBarEntry
{
    ...
    uint32 m_MinValue;                                      // 1
    uint32 m_MaxValue;                                      // 2
    uint32 m_DefaultValue;                                  // 3
    // ... many unk values and descriptions
};
