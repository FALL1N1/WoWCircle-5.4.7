...
void ChatHandler::FillMessageData(WorldPacket* data, WorldSession* session, uint8 type, uint32 language, const char *channelName, uint64 target_guid, const char *message, Unit* speaker, const char* addonPrefix /*= NULL*/, uint32 achievementId /*= 0*/)
{
    ...
    if (target_guid && session)
    {
        if (Unit* unit = ObjectAccessor::GetUnit(*session->GetPlayer(), target_guid))
        {
...
GameObject* ChatHandler::GetObjectGlobalyWithGuidOrNearWithDbGuid(uint32 lowguid, uint32 entry)
{
    ...
    GameObject* obj = pl->GetMap()->GetGameObject(MAKE_NEW_GUID(lowguid, entry, HighGuid::GameObject));
    ...
    switch (type)
    {
        ...
        case SPELL_LINK_CREATURE:
        {
            ...
            if (CreatureData const* data = sObjectMgr->GetCreatureData(lowguid))
                return MAKE_NEW_GUID(lowguid, data->id, HighGuid::Unit);
            else
                return 0;
        }
        case SPELL_LINK_GAMEOBJECT:
        {
            ...
            if (GameObjectData const* data = sObjectMgr->GetGOData(lowguid))
                return MAKE_NEW_GUID(lowguid, data->id, HighGuid::GameObject);
            else
                return 0;
        }
    }
