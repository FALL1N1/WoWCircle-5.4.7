            ...
            for (auto& spawnId : p.second)
            {
                ...
                for (auto itr = creatureBounds.first; itr != creatureBounds.second; ++itr)
                {
                    ...
                    if (CreatureTemplate const* creatureTemplate = creature->GetCreatureTemplate())
                    {
                        ...
                        npcflag2 |= creatureTemplate->npcflag2;
                    }

                    creature->SetUInt32Value(UNIT_NPC_FLAGS, npcflag);
                    creature->SetUInt32Value(UNIT_NPC_FLAGS + 1, npcflag2);
...
public:
    ...
    void Visit(std::unordered_map<uint64, Creature*>& creatureMap)
    {
    ...
    void Visit(std::unordered_map<uint64, GameObject*>& gameObjectMap)
    {
    ...
    template<class T>
    void Visit(std::unordered_map<uint64, T*>&) { }
