...
class Item : public Object
{
    public:
        ...
        virtual bool LoadFromDB(uint32 guid, uint64 owner_guid, Field* fields, uint32 entry, uint32 transEntry = 0);
        ...
        // Custom Transmogrigy
        FakeResult CanFakeDisplay(uint32 iEntry, bool withPay = false);
        uint32 GetTransEntry() const { return _transEntry; }
        void SetTransEntry(uint32 val) { _transEntry = val; }
        ...
        inline void SetText(std::string const& text)
        {
            ...
            for (uint64 i = 0; i < m_text.size(); i++)
            {
                ...
                {
                    sLog->OutTrinity("Item::SetText: Incorrect characters %s, m_text set to NULL", m_text.c_str());
                    m_text = "";
                    return;
                }
            }
        ...
        int32 GetReforgableStat(ItemModType statType, Player* player = nullptr, uint32 ilvl = 0) const;

        bool IsPvPItem() const;
        bool IsStuffItem() const;
        bool IsLegendaryCloak() const;

        void BuildItemModifiers(ByteBuffer* data) const;
