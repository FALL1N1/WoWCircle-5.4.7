...
class Object
{
    public:
        ...
        uint32 GetGUIDHigh() const { return (uint32)GUID_HIPART(GetUInt64Value(0)); }
        ...
        void SetUInt16Value(uint16 index, uint8 offset, uint16 value, bool forced = false);
        ...
        void SetDynamicUInt32Value(uint32 tab, uint16 index, uint32 value, bool forced = false);
...
class WorldObject : public Object, public WorldLocation
{
    ...
    public:
        ...
        TempSummon* SummonCreature(uint32 id, const Position &pos, TempSummonType spwtype = TEMPSUMMON_MANUAL_DESPAWN, uint32 despwtime = 0, uint32 vehId = 0, uint64 viewerGuid = 0, std::list<uint64>* viewersList = NULL, SummonPropertiesEntry const* p_Properties = nullptr) const;
        TempSummon* SummonCreature(uint32 id, float x, float y, float z, float ang = 0, TempSummonType spwtype = TEMPSUMMON_MANUAL_DESPAWN, uint32 despwtime = 0, uint64 viewerGuid = 0, std::list<uint64>* viewersList = NULL, SummonPropertiesEntry const* p_Properties = nullptr)
        {
            if (!x && !y && !z)
            {
            ...
            return SummonCreature(id, pos, spwtype, despwtime, 0, viewerGuid, viewersList, p_Properties);
        }
