...
bool Pet::LoadPetFromDB(Player* owner, uint32 petentry, uint32 petnumber, bool current, PetSlot slotID, bool stampeded, PetLoginQueryHolder* holder, bool login)
{
    ...
    {
        if (owner->getClass() == CLASS_WARLOCK && owner->ToPlayer()->GetSpecializationId() != SPEC_WARLOCK_DEMONOLOGY)
            return false;
    }
...
void Pet::SavePetToDB(PetSlot mode, bool stampeded)
{
    ...
    if (mode == PET_SLOT_OTHER_PET) // warlock
    {
        uint32 ownerLowGUID = GUID_LOPART(GetOwnerGUID());
        std::string name = m_name;
        CharacterDatabase.EscapeString(name);
        trans = CharacterDatabase.BeginTransaction();
        // correct way to remove data for warlock
        trans->PAppend("DELETE FROM character_pet WHERE owner = '%u'", ownerLowGUID);

        // save pet
        std::ostringstream ss;
        ss << "INSERT INTO character_pet (id, entry, owner, modelid, level, Reactstate, slot, name, renamed, curhealth, curmana, abdata, savetime, CreatedBySpell, PetType, specialization) "
            << "VALUES ("
    ...
    switch (m_deathState)
    {
        case CORPSE:
        {
            ...
            if (Player* owner = GetOwner())
            {
                if (!m_Stampeded)
                {
                    if (!owner->HasSpell(109212) || (owner->HasSpell(109212) && owner->HasAura(118694)))
        ...
        default:
            if (!isAlive())
            {
                ...
                {
                    // Spirit Bond
                    if (!owner->HasSpell(109212) || (owner->HasSpell(109212) && owner->HasAura(118694) && !m_Stampeded))
                        owner->RemoveAura(118694);
...
void Creature::Regenerate(Powers power)
{
    if (IsAIEnabled)
    {
        if (!AI()->CanRegeneratePower(power))
            return;
    }
    ...
    switch (petType)
    {
        ...
        default:
        {
            switch (GetEntry())
            {
                ...
                case ENTRY_XUEN: // Xuen, the White Tiger
                {
...
void Pet::_LoadAuras(PreparedQueryResult auraResult, PreparedQueryResult auraEffectResult, uint32 timediff, bool login)
{
    ...
    if (resultEffect)
    {
        do
        {
            ...
            auraEffectList.push_back(auraEffectData(slot, effect, amount, baseamount, data));
        }
    ...
    if (result)
    {
        do
        {
            ...
            if (aura != nullptr)
            {
                ...
                aura->SetLoadedState(maxduration, remaintime, remaincharges, stackcount, recalculatemask, &damage[0], &data[0]);
    ...
    for (AuraMap::const_iterator itr = m_ownedAuras.begin(); itr != m_ownedAuras.end(); ++itr)
    {
        ...
        for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
        {
            if (aura->GetEffect(i))
            {
                ...
                stmt->setString(index++, aura->GetEffect(i)->GetData());
...
void Pet::LearnPetPassives()
{
    ...
    // Apply passive aura for owners talent
    CastSpell(this, 53397, true);
    CastSpell(this, 134732, true);
}
