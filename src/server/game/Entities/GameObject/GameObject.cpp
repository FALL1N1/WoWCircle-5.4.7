...
GameObject::GameObject() : WorldObject(false), MapObject(), m_model(NULL), m_goValue(new GameObjectValue), m_AI(NULL)
{
...
bool GameObject::Create(uint32 guidlow, uint32 name_id, Map* map, uint32 phaseMask, float x, float y, float z, float ang, float rotation0, float rotation1, float rotation2, float rotation3, uint32 animprogress, GOState go_state, uint32 artKit)
{
    ...
    {
        sLog->outError(LOG_FILTER_SQL, "Gameobject (GUID: %u Entry: %u) not created: gameobject type GAMEOBJECT_TYPE_MO_TRANSPORT cannot be manually created.", guidlow, name_id);
        return false;
    }
    ...
    switch (m_lootState)
    {
        case GO_NOT_READY:
        {
            switch (GetGoType())
            {
                case GAMEOBJECT_TYPE_TRAP:
                {
                    ...
                    {
                        // TODO
                        // Patch 3.0.2: The arming time has been reduced to 1 second, from 2 seconds for all traps.
        ...
        case GO_READY:
        {
            if (m_respawnTime > 0)                          // timer on
            {
                ...
                {
                    uint64 dbtableHighGuid = MAKE_NEW_GUID(m_spawnId, GetEntry(), HighGuid::GameObject);
            ...
            if (isSpawned())
            {
                ...
                if (goInfo->type == GAMEOBJECT_TYPE_TRAP)
                {
                    ...
                    if (ok)
                    {
                        ...
                        if (Creature* creature = ok->ToCreature())
...
bool GameObject::LoadGameObjectFromDB(uint32 spawnId, Map* map, bool addToMap)
{
    ...
    if (!data)
    {
        sLog->outError(LOG_FILTER_SQL, "Gameobject (GUID: %u) not found in table `gameobject`, can't load. ", spawnId);
        return false;
    }
    ...
    switch (GetGoType())
    {
        ...
        case GAMEOBJECT_TYPE_GOOBER:                        //10
        {
            ...
            if (user->GetTypeId() == TYPEID_PLAYER)
            {
                ...
                {
                    sLog->outDebug(LOG_FILTER_MAPSCRIPTS, "Goober ScriptStart id %u for GO entry %u (GUID %u).", info->goober.eventId, GetEntry(), GetSpawnId());
                ...
                GetMap()->ScriptsStart(sGameObjectScripts, GetSpawnId(), player, this);
            }
...
void GameObject::SendGameObjectActivateAnimKit(uint32 p_AnimKitID, bool p_Maintain /*= true*/)
{
    ObjectGuid l_Guid = GetGUID();

    WorldPacket l_Data(SMSG_GAMEOBJECT_ACTIVATE_ANIM_KIT, 8 + 4);
...

        {
            ...
            if (index == OBJECT_FIELD_DYNAMIC_FLAGS)
            {
                ...
                switch (GetGoType())
                {
                    ...
                    case GAMEOBJECT_TYPE_TRANSPORT:
                        {
                            if (uint32 transportPeriod = ToGameObject()->GetTransportPeriod())
                            {
                                float timer = float(ToGameObject()->GetGOValue()->Transport.PathProgress % transportPeriod);
