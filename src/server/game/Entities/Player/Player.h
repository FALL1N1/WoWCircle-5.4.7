...
#include "../../Cinematic/CinematicPathMgr.h"
...
{
    auraEffectData(uint8 slot, uint8 effect, int32 amount, int32 baseamount, std::string data) : _slot(slot), _effect(effect), _amount(amount), _baseamount(baseamount), _data(data) {}
    uint8 _slot;
    uint8 _effect;
    int32 _amount;
    int32 _baseamount;
...
typedef std::map<uint32, uint32> QuestCurrenciesAmount;
...
class Player : public Unit, public GridObject<Player>
{
    ...
    public:
        ...
        void ResetSocial() { m_social = nullptr; }
        ...
        void SetGMRaidTarget(uint64 targetGuid) { m_GMRaidTarget = targetGuid; }
        uint64 GetGMRaidTarget() const { return m_GMRaidTarget; }
        ...
        bool isSpectateRemoving() const { return spectateRemoving; }
        void SetSpectateRemoving(bool _value) { spectateRemoving = _value; }
        void SetSpectate(bool on);
        bool isAwaitingToBG() { return IsAwaitingToBG; }
        void SetAwaitingToBG(bool _set) { IsAwaitingToBG = _set; }
        ...
        void Whisper(const std::string& text, const uint32 language, Player *player);
        ...
        PlayerCurrenciesMap& GetCurrencyStorage() { return _currencyStorage; }
        void SaveCurrency();
        ...
        */
        ...
        void UpdateQuestCurrencies(uint32 p_QuestId, uint32 p_Type, uint32 p_Count);
        uint32 GetQuestCurrencyMaxAmount(uint32 p_QuestID);

        bool Match(PlayerConditionEntry const* condition);
        ...
        void SaveKillsToDB(SQLTransaction& trans);
        void SaveQuestStatus(SQLTransaction& trans);
        ...
        void ResetRegenTimer(Powers power);
        ...
        QuestStatusSaveMap& getRewardedQuestsSave() { return m_RewardedQuestsSave; }
        ...
        PlayerMails const& GetPlayerMails() { return m_mail; }
        ...
        void learnSpell(uint32 spell_id, bool dependent, bool learn = true);
        void removeSpell(uint32 spell_id, bool disabled = false, bool learn_low_rank = true, bool send = true);
        ...
        uint32 GetRoleConsideringSpe(uint32 p_SpecializationId = 0);
        ...
        bool UnlearnTalent(uint32 talentId);
        bool LearnTalentUnsafe(uint32 talentId, uint32 spellId, bool learn = true);
        ...
        void ResetSpec();
        void ResetSpecialization_Internal();
        ...
        */
        ...
        bool HandleGlyph(uint32 glyph, uint32 glyphIndex, bool apply, bool cast = true);
        ...
        void AddChargeCooldown(SpellInfo const* spellInfo);
        ...
        ActionButton const* GetActionButton(uint8 button) const;
        void SendInitialActionButtons() const { SendActionButtons(0); }
        void SendActionButtons(uint32 state) const;
        bool IsActionButtonDataValid(uint8 button, uint32 action, uint8 type);

        int8 GetFreeActionButton() const;
        ...
        void SetArenaPersonalRating(uint8 slot, int32 value)
        {
            ...
            if (m_BestRatingOfWeek[slot] < uint32(value))
                m_BestRatingOfWeek[slot] = value;
            if (m_BestRatingOfSeason[slot] < uint32(value))
                m_BestRatingOfSeason[slot] = value;
        }

        void SetArenaMatchMakerRating(uint8 slot, int32 value)
        {
            ASSERT(slot < MAX_PVP_SLOT);
        ...
        uint32 GetBaseRatingMod(CombatRating cr) const { return m_baseRatingValue[cr]; }
        ...
        void SendAttackSwingError(AttackSwingErrors error);
        ...
        Corpse* CreateCorpse();
        void KillPlayer();
        static void OfflineResurrect(uint64 guid, SQLTransaction& trans);
        ...
        SkillStatusMap& GetSkillsMap() { return mSkillStatus; }
        ...
        void _ApplyItemBonuses(Item const* item, uint8 slot, bool apply, bool only_level_scale = false, uint32 rescaleToItemLevel = 0);
        void _ApplyWeaponDamage(uint8 slot, Item const* item, ScalingStatValuesEntry const* ssv, bool apply, uint32 minDamage = 0, uint32 maxDamage = 0);
        ...
        bool UpdateItemLevelCutOff(uint32 p_MinLevel, uint32 p_MaxLevel, bool p_RescaleItems = true);
        ...
        {
            for (uint8 i = 0; i < PLAYER_MAX_BATTLEGROUND_QUEUES; ++i)
                for (uint8 queueId = BATTLEGROUND_QUEUE_2v2; queueId <= BATTLEGROUND_QUEUE_5v5; ++queueId)
                    if (m_bgBattlegroundQueueID[i].bgQueueTypeId == queueId)
                        return true;
            return false;
        }
        ...
        uint32 m_deserterApplyTime;
        uint8 m_deserterApplyCount;

        void CastDeserterDebuff();
        ...
        PetSlot getSlotForNewPet() const
        {
            uint32 l_LastKnown = 0;
            ...
            for (uint32 l_I = uint32(PET_SLOT_HUNTER_FIRST); l_I < l_LastKnown; ++l_I)
            {
        ...
        typename std::enable_if<std::is_floating_point<T>::value, T>::type
            GetRate(RateType type)
        {
            ASSERT(type < RateType::Max);
            return m_rates[type].fval;
        }
        ...
        void UpdateAchievementCriteria(AchievementCriteriaTypes type, uint64 miscValue1 = 0, uint64 miscValue2 = 0, uint64 miscValue3 = 0, Unit* unit = NULL, bool p_LoginCheck = false, bool sync = false);
        ...
        */
        ...
        float GetCollisionHeight(bool mounted)
        {
            if (mounted)
            {
                ...
                if (GetUInt32Value(UNIT_FIELD_MOUNTDISPLAYID) == 53774)
                    scaleMod = 0.3f;
        ...
        uint32 GetItemScaleLevel(uint8 slot) const
        {
            return m_itemScale[slot];
        }
        ...
        bool m_HasDarkSimulacrumAura; // Cache result in this variable, to improve performance of IsDarkSimulacrum function

        time_t m_bonusFactionLastTime;
        ...
        void IncrementCounter(uint32 entry);
        void DecrementCounter(uint32 entry);

        uint32 GetBagsFreeSlots() const;

        ACE_Thread_Mutex m_DeleteLock;

        void UpdateHeirloomBonus(Item* changed);
    ...
    private:
        struct BGTalentInfo
        {
            struct Glyph
            {
                ...
                Glyph(uint32 _index, uint32 _glyph)
                    : index(_index), glyph(_glyph)
                {
                }
            };

            typedef std::vector<Glyph> vGlyphs;
            typedef std::vector<uint32> vTalents;

            vGlyphs Glyphs[MAX_TALENT_SPECS];
            vTalents Talents[MAX_TALENT_SPECS];
            uint32 SpecializationIndex;
            ...
            struct spell_to_learn_comp
            {
                spell_to_learn_comp(Player const* player, uint8 const spec)
                    : m_player(player), m_spec(spec)
                {

                }

                bool operator()(const uint32 spell) { return !m_player->HasTalent(spell, m_spec); }

                uint8 const m_spec;
                Player const* m_player;
            };

            struct glyph_to_learn_comp
            {
                glyph_to_learn_comp(Player const* player, uint8 const spec)
                    : m_player(player), m_spec(spec)
                {

                }

                bool operator()(Glyph const& glyph) { return m_player->GetGlyph(m_spec, glyph.index) != glyph.glyph; }

                uint8 const m_spec;
                Player const* m_player;
            };

            struct glyph_special_compare : public std::unary_function<Glyph, bool>
            {
                explicit glyph_special_compare(uint32 const glyphId, uint32 const glyphIndex)
                    : m_glyphId(glyphId), m_glyphIndex(glyphIndex)
                {

                }

                bool operator() (Glyph const& arg)
                {
                    return arg.glyph == m_glyphId && arg.index == m_glyphIndex;
                }

                uint32 const m_glyphId;
                uint32 const m_glyphIndex;
            };
        };

        BGTalentInfo* m_bgTalentInfo;
        ...
        // Summons counter
        std::unordered_map<uint32, uint32> m_summonCounter;
        ...
        */
        ...
        void _LoadQuestCurrency(PreparedQueryResult p_Result);
        ...
        std::map<uint32, QuestCurrenciesAmount> m_PlayerQuestCurrency;
...
//        Spell* m_spellModTakingSpell;  // Spell for which charges are dropped in spell::finish
        ...
        std::array<intfloat, RateType::Max> m_rates;
    ...
    public:
        ...
        void LoadAfterInterRealm(QueryResult result);
        void LoadAfterInterRealmCurrencies(QueryResult result);
        void LoadAfterInterRealmItems(QueryResult result);
        void LoadAfterInterRealmSpellCooldowns(QueryResult result);
        void LoadAfterInterRealmCUFProfiles(QueryResult result);
        void LoadAfterInterRealmRandomBGStatus(QueryResult result);
        void LoadAfterInterRealmActions(QueryResult result);
        void LoadAfterInterRealmEquipmentSets(QueryResult result);
        void LoadAfterInterRealmSocialList(QueryResult result);
        void LoadAfterInterRealmAuras(QueryResult result, QueryResult resultEffect);
        void LoadAfterInterRealmArenaStats(QueryResult result);
        ...
        //static bool HandlePinfoCommand(ChatHandler *handler, Player *player, uint64 const &playerGUID, std::string const &playerName);
        //static bool HandlePinfoCommand(Player *receiver, Player *player, uint64 const &playerGUID, std::string const &playerName);
...
template <class T> T Player::ApplySpellMod(uint32 spellId, SpellModOp op, T &basevalue, Spell* spell, bool removestacks)
{
    ...
    for (SpellModList::iterator itr = m_spellMods[op].begin(); itr != m_spellMods[op].end(); ++itr)
    {
        ...
        if (mod->ownerAura)
        {
            const SpellInfo* temp = mod->ownerAura->GetSpellInfo();
            if (temp)
        ...
        else if (mod->type == SPELLMOD_PCT)
        {
            ...
            // Fix Clearcasting not being consumed if Predatory Swiftness is there
            if (mod->spellId == 135700) //< Clearcasting
            {
                // Predatory Swiftness gets consumed first, preventing Omen of Clarity from proc
                // Confirmed by classmaster and video
                // https://www.youtube.com/watch?v=6QC7vOBGUpY&feature=youtu.be&t=12m19s
                if (AuraEffect* predatorySwiftness = GetAuraEffect(69369, EFFECT_0)) // Predatory Swiftness
                {
                    if (predatorySwiftness->IsAffectingSpell(spellInfo))
                        continue;
                }
            }
            ...
            {
                if (mod->ownerAura && mod->ownerAura->GetCharges() < 3) ///< Backdraft applies on Chaos Bolt only with more than 3 charges
                    value = 0;
        ...
        // Hack fix Borrowed Time
        if (mod->spellId == 59889 && !IsNeedToRemoveBorrowedTime())
            removestacks = false;
