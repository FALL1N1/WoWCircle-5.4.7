...
void Corpse::DeleteFromDB(uint64 ownerGuid, SQLTransaction& trans)
{
...
bool Corpse::LoadCorpseFromDB(uint32 guid, Field* fields)
{
    ...
    // SELECT posX, posY, posZ, orientation, mapId, displayId, itemCache, bytes1, bytes2, flags, dynFlags, time, corpseType, instanceId, phaseMask, guid FROM corpse WHERE corpseType <> 0
    ...
    SetUInt64Value(CORPSE_FIELD_OWNER, MAKE_NEW_GUID(ownerGuid, 0, HighGuid::Player));
