...
uint32 BlackMarketMgr::GetAuctionOutBid(uint64 bid)
{
...
{
    data.Initialize(SMSG_BLACKMARKET_ITEM_RESULT, 4 + 3 + (1*8)/8 + 8*52);
    ...
    for (auto itr : BMAuctionsMap)
    {
        ...
        data << uint64(nextBidPrice);
        data << uint64(currentBid);
        data << uint32(0);                                 // Unk
        data << uint32(auction->id);
        data << uint32(auction->bm_template->seller);
        data << uint64(upPrice);
        data << uint32(auction->TimeLeft());
        data << uint32(auction->bidderCount);
        data << uint32(auction->bm_template->itemCount);
    }

    if (count)
        data.PutBits(count, 18, wpos, bpos);
...
std::string BMAuctionEntry::BuildAuctionMailBody(uint32 lowGuid)
{
    std::ostringstream strm;
    strm.width(16);
    strm << std::right << std::hex << MAKE_NEW_GUID(lowGuid, 0, HighGuid::Player);   // HighGuid::Player always present, even for empty guids
    strm << std::dec << ':' << bid << ':' << 0;
    strm << ':' << 0 << ':' << 0;
    return strm.str();
}
...
{
    Player* bidder = sObjectAccessor->FindPlayer(MAKE_NEW_GUID(auction->bidder, 0, HighGuid::Player));
    if (bidder)
    {
...
{
    uint64 bidderGUID = MAKE_NEW_GUID(auction->bidder, 0, HighGuid::Player);
