...
void WorldSession::HandlePetitionShowSignOpcode(WorldPacket& recvData)
{
    ...
    WorldPacket data(SMSG_PETITION_SHOW_SIGNATURES, 1 + 1 + 3 + 4 + 8 + 8 + 12*signs + 1*signs);
    ByteBuffer signsBuffer(12*signs);
    ...
    for (uint8 i = 1; i <= signs; ++i)
    {
        ...
        ObjectGuid signerGuid = MAKE_NEW_GUID(lowGuid, 0, HighGuid::Player);
...
void WorldSession::HandlePetitionSignOpcode(WorldPacket& recvData)
{
    ...
    fields = result->Fetch();
    ObjectGuid ownerGuid = MAKE_NEW_GUID(fields[0].GetUInt32(), 0, HighGuid::Player);
...
void WorldSession::HandleOfferPetitionOpcode(WorldPacket& recvData)
{
    ...
    if (type == GUILD_CHARTER_TYPE)
    {
        ...
        {
            SendPetitionSignResult(_player->GetGUID(), MAKE_NEW_GUID(petitionGuidLow, 0, HighGuid::Item), PETITION_SIGN_ALREADY_SIGNED_OTHER);
            return;
        }

        if (player->GetGuildId())
        {
            SendPetitionSignResult(_player->GetGUID(), MAKE_NEW_GUID(petitionGuidLow, 0, HighGuid::Item), PETITION_SIGN_ALREADY_IN_GUILD);
            return;
        }
    }
    ...
    WorldPacket data(SMSG_PETITION_SHOW_SIGNATURES, 1 + 1 + 3 + 4 + 8 + 8 + 13*loParts.size());
    ByteBuffer signsBuffer(loParts.size()*12);
