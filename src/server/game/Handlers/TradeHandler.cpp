...
{
    WorldPacket data(SMSG_TRADE_STATUS, 1 + 4);
...
void WorldSession::SendUpdateTrade(bool trader_data /*= true*/)
{
    ...
    WorldPacket data(SMSG_TRADE_UPDATED, 48);
    data << uint64(view_trade->GetMoney());
    data << uint32(0);
    ...
    for (uint8 i = 0; i < TRADE_SLOT_COUNT; ++i)
    {
        ...
        if (notWrapped)
        {
            ...
            item->BuildItemModifiers(&itemData);
...
void WorldSession::HandleInitiateTradeOpcode(WorldPacket& recvPacket)
{
    ...
    if (pOther->GetSocial() && pOther->GetSocial()->HasIgnore(GetPlayer()->GetGUIDLow()))
    {
    ...
    WorldPacket data(SMSG_TRADE_STATUS, 1 + 1 + 8);
