    ...
    for (uint8 i = 0; i < researchDataCount; ++i)
    {
        switch (archeologyType[i])
        {
            case 1: // Fragments
                ...
                _player->GetArchaeologyMgr().AddProjectCost(entry[i], usedCount[i], true);
                break;
            case 2: // Keystones
                recvPacket >> entry[i];     // Item ID
                recvPacket >> usedCount[i]; // ItemCount
                _player->GetArchaeologyMgr().AddProjectCost(entry[i], usedCount[i], false);
                break;
            default:
                break;
        }
...
void WorldSession::HandleCastSpellOpcode(WorldPacket& recvPacket)
{
    ...
    for (uint8 i = 0; i < researchDataCount; ++i)
    {
        uint32 entry = recvPacket.read<uint32>();
        uint32 count = recvPacket.read<uint32>();
        uint8 type = archeologyType[i];

        _player->GetArchaeologyMgr().AddProjectCost(entry, count, type == 1 ? true : false);
    }
    ...
    if (mover->GetTypeId() == TYPEID_PLAYER)
    {
        ...
        if ((!mover->ToPlayer()->HasActiveSpell(spellId) || spellInfo->IsPassive()) 
            ...
            && !(IS_GAMEOBJECT_GUID(targetGuid) && spellInfo->HasEffect(SPELL_EFFECT_OPEN_LOCK))
            && !spellInfo->IsAbilityOfSkillType(SKILL_ARCHAEOLOGY) && !spellInfo->IsCustomArchaeologySpell() && !spellInfo->HasEffect(SPELL_EFFECT_LOOT_BONUS))
        {
            //cheater? kick? ban?
            recvPacket.rfinish(); // prevent spam at ignore packet
            return;
    ...
    {
        if (targets.GetUnitTarget()->GetGUID() != _player->GetGUID() && !_player->IsFriendlyTo(targets.GetUnitTarget()))
        {
    ...
    else if (spellInfo->Id == 6229 && _player->getClass() == CLASS_WARLOCK)
    {
        if (_player->HasAura(114168))
        {
            SpellInfo const* newSpellInfo = sSpellMgr->GetSpellInfo(119839);
            if (newSpellInfo)
            {
                spellInfo = newSpellInfo;
                spellId = newSpellInfo->Id;
    ...
    //if ((spellInfo->GetExplicitTargetMask() & TARGET_FLAG_DEST_LOCATION) && !hasDestLocation)
        //return;

    Spell* spell = new Spell(mover, spellInfo, l_TriggerFlags, 0, false);
...
void WorldSession::HandleMirrorImageDataRequest(WorldPacket& recvData)
{
    ...
    // Get effect data (containing the appareance)
    std::string effectData = unit->GetAuraEffectsByType(SPELL_AURA_CLONE_CASTER).front()->GetData();
    AppareanceData* appareanceData = reinterpret_cast<AppareanceData*>(&effectData[0]);
    if (effectData.size() != sizeof(AppareanceData) || appareanceData->Version != AppareanceData::GetCurrentVersion())
        appareanceData->Save(unit); // Shouldn't happen but let's deal with this anyway

    WorldPacket data(SMSG_MIRROR_IMAGE_DATA, 68);

    if (appareanceData->IsPlayer)
    {
        ObjectGuid guildGuid(appareanceData->GuildGuid);
        ...
        data << uint8(appareanceData->HairStyle);
        data.WriteByteSeq(guid[1]);
        data.WriteByteSeq(guid[7]);
        data.WriteByteSeq(guildGuid[5]);
        data << uint8(appareanceData->Race);
        data.WriteByteSeq(guildGuid[6]);
        data << uint8(appareanceData->HairColor);
        ...
        for (EquipmentSlots const* itr = &itemSlots[0]; *itr != EQUIPMENT_SLOT_END; ++itr)
            data << uint32(appareanceData->EquipmentDisplayId[*itr]);
        ...
        data << uint32(appareanceData->DisplayId);
        data << uint8(appareanceData->SkinColor);
        data << uint8(appareanceData->Class);
        data << uint8(appareanceData->FaceId);
        data.WriteByteSeq(guildGuid[1]);
        data << uint8(appareanceData->Gender);
        data.WriteByteSeq(guildGuid[4]);
        data << uint8(appareanceData->FacialHair);
