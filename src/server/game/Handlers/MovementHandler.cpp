...
void WorldSession::HandleMoveWorldportAckOpcode()
{
    ...
    if (GetPlayer()->HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_GHOST) || (corpse && corpse->GetType() != CORPSE_BONES))
    {
        if (mEntry->IsDungeon())
        {
    ...
    if (mInstance)
    {
        ...
        if (MapDifficulty const* mapDiff = GetMapDifficultyData(mEntry->MapID, diff))
        {
            ...
            {
                /// Prefer sending the real encounters reset time if player is binded to the raid.
                RaidEncounter* l_Encounter = sRaidEncountersMgr->GetEncounterForPlayerAndMap(_player->GetGUIDLow(), mEntry->MapID);
                if (l_Encounter != nullptr)
                    _player->SendInstanceResetWarning(mEntry->MapID, diff, sWorld->getWorldState(WS_RAID_ENCOUNTERS_RESET_TIME) - time(nullptr));
                else if (time_t timeReset = sInstanceSaveMgr->GetResetTimeFor(mEntry->MapID, diff))
                {
    ...
    GetPlayer()->SetTeleportFlagForAnticheat(true);
}
...
void WorldSession::HandleMoveTeleportAck(WorldPacket& recvPacket)
{
    ...
    plMover->SetTeleportFlagForAnticheat(true);
}
...
void WorldSession::HandleMovementOpcodes(WorldPacket& recvPacket)
{
    ...
    if (movementInfo.transport.guid)
    {
        ...
        if (!JadeCore::IsValidMapCoord(movementInfo.pos.GetPositionX() + movementInfo.transport.pos.GetPositionX(), movementInfo.pos.GetPositionY() + movementInfo.transport.pos.GetPositionY(),
    ...
    if (plrMover && movementInfo.HasMovementFlag(MOVEMENTFLAG_MASK_MOVING))
    {
        if (uint32 l_EmoteId = plrMover->GetUInt32Value(UNIT_NPC_EMOTESTATE))
        {
            if (EmotesEntry const* emoteInfo = sEmotesStore.LookupEntry(l_EmoteId))
            {
                if (emoteInfo->EmoteType != 1)
                    plrMover->SetUInt32Value(UNIT_NPC_EMOTESTATE, 0);
            }
        }
    }
...
void WorldSession::HandleMoveKnockBackAck(WorldPacket & recvData)
{
    ...
    Position old;
    _player->GetPosition(&old);

    _player->m_movementInfo = movementInfo;
    _player->UpdatePosition(old);

    
    if (_player->HasAura(34480))
    {
        _player->CastSpell(_player, 39432, true);
    }
...
void WorldSession::HandleSummonResponseOpcode(WorldPacket& recvData)
{
    ...
    _player->SetTeleportFlagForAnticheat(true);
