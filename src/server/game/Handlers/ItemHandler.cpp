...
void WorldSession::HandleSplitItemOpcode(WorldPacket& recvData)
{
    ...
    {
        if (Spell* spell = _player->GetCurrentSpell(i))
        {
            if (spell->m_castItemGUID)
            {
                _player->SendEquipError(EQUIP_ERR_CLIENT_LOCKED_OUT, NULL, NULL);
                return;
            }
        }
    }
...
void WorldSession::SendItemDb2Reply(uint32 entry)
{
    ...
    WorldPacket data(SMSG_DB_REPLY, proto ? 48 : 16);
    if (!proto)
    {
        data << uint32(-1);         // entry
...
void WorldSession::SendItemSparseDb2Reply(uint32 entry)
{
    ...
    for (uint32 x = 0; x < MAX_ITEM_PROTO_STATS; ++x)
        buff << int32(proto->ItemStat[x].ItemScalingValue);

    for (uint32 x = 0; x < MAX_ITEM_PROTO_STATS; ++x)
        buff << float(proto->ItemStat[x].ItemSocketCostRate);
...
void WorldSession::SendItemUpgradeReply(uint32 entry)
{
    ItemUpgradeEntry const* proto = sItemUpgradeStore.LookupEntry(entry);

    WorldPacket data(SMSG_DB_REPLY, 16 + proto ? 24 : 0);
    if (!proto)
    {
        data << uint32(-1);
        data << uint32(DB2_REPLY_ITEM_UPGRADE);
    ...
    data << uint32(entry);
    data << uint32(DB2_REPLY_ITEM_UPGRADE);
    data << uint32(sObjectMgr->GetHotfixDate(entry, DB2_REPLY_ITEM_UPGRADE));
    data << uint32(sizeof(ItemUpgradeEntry));
    data.append((const char*)proto, sizeof(ItemUpgradeEntry));
    SendPacket(&data);
}
...
void WorldSession::HandleTransmogrifyItems(WorldPacket& recvData)
{
    ...
    for (uint8 i = 0; i < count; ++i)
    {
        ...
        if (newEntries[i])
        {
            ...
            if (!player->HasItemCount(newEntries[i], 1, true))
                return;
        }
...
void WorldSession::HandleReforgeItemOpcode(WorldPacket& recvData)
{
    ...
    if (!item->GetReforgableStat(ItemModType(stats->SourceStat), nullptr, 0) || item->GetReforgableStat(ItemModType(stats->FinalStat), nullptr, 0)) // Cheating, you cant reforge to a stat that the item already has, nor reforge from a stat that the item does not have
    {
        SendReforgeResult(false);
        return;
    }
...
void WorldSession::HandleUpgradeItemOpcode(WorldPacket& recvData)
{
    ...
    ObjectGuid npcGuid, guid1;
    ObjectGuid itemGuid, guid2;
    Player* player = GetPlayer();
    ...
    if (!item->GetTemplate()->CanUpgrade())
    {
        sLog->outDebug(LOG_FILTER_NETWORKIO, "WORLD: HandleUpgradeItemOpcode - Trying to upgrade not-upgradeable item %u.", item->GetEntry());
        SendItemUpgradeResult(false);
        return;
    }
    ...
    if (item->IsEquipped() && !item->IsBroken())
        player->RescaleItemTo(item->GetSlot(), player->GetEquipItemLevelFor(item->GetTemplate(), item));
...
void WorldSession::HandleSetLootSpecialization(WorldPacket& recvData)
{
    ...
    _player->SetLootSpecId(specializationId);
    _player->SetUInt32Value(PLAYER_FIELD_LOOT_SPEC_ID, specializationId);
}
