...
{
    QUEST_SPECIAL_FLAGS_NONE                 = 0x0000,
    // Trinity flags for set SpecialFlags in DB if required but used only at server
    QUEST_SPECIAL_FLAGS_REPEATABLE           = 0x0001,
    QUEST_SPECIAL_FLAGS_EXPLORATION_OR_EVENT = 0x0002, // if required area explore, spell SPELL_EFFECT_QUEST_COMPLETE casting, table `*_script` command SCRIPT_COMMAND_QUEST_EXPLORED use, set from script)
    QUEST_SPECIAL_FLAGS_AUTO_ACCEPT          = 0x0004, // quest is to be auto-accepted.
    QUEST_SPECIAL_FLAGS_DF_QUEST             = 0x0008, // quest is used by Dungeon Finder.
    ...
    QUEST_SPECIAL_FLAGS_DYNAMIC_ITEM_REWARD  = 0x0040, // Set by 64  in SpecialFlags in DB if the quest has dynamic item reward
    QUEST_SPECIAL_FLAGS_ADD_QUEST_BONUS      = 0x0080, // Set by 128 in SpecialFlags in DB if the quest always add bonus for overriden reputation
    ...
    QUEST_SPECIAL_FLAGS_DB_ALLOWED =
        ...
        QUEST_SPECIAL_FLAGS_DYNAMIC_ITEM_REWARD |
        QUEST_SPECIAL_FLAGS_ADD_QUEST_BONUS,
    ...
    QUEST_SPECIAL_FLAGS_KILL_OR_CAST         = 0x0400,   // Internal flag computed only
