...
enum SpellTargetFlagCu
{
    SPELL_TARGET_FLAG_CU0_SORT_BY_RANGE = 0x1, // sort targets by range
    SPELL_TARGET_FLAG_CU0_SORT_BY_HEALTH = 0x2, // sort targets by hp
    SPELL_TARGET_FLAG_CU0_RESIZE_ONLY = 0x4, // just resize targets list
    SPELL_TARGET_FLAG_CU0_EXCLUDE_CASTER = 0x8, // exclude caster from targets
    SPELL_TARGET_FLAG_CU0_PARTY_OR_RAID = 0x10, // raid or party member
    SPELL_TARGET_FLAG_CU0_NPC_ONLY = 0x20, // select only non-player object
};
...
public:
    ...
    int32 CalcValue(Unit const* caster = NULL, int32 const* basePoints = NULL, Unit const* target = NULL, Item const* itemTarget = nullptr, int32 itemLevel = -1) const;
...
public:
    ...
    bool IsTargetingArea() const;
    bool IsTargetingEnemySingle() const;
    ...
    inline bool HasCustomTargetFlag(SpellTargetFlagCu customFlag) const { return TargetFlagCu & customFlag; }
    ...
    SpellCastResult CheckExplicitTarget(Unit const* caster, WorldObject const* target, Item const* itemTarget = NULL, volatile int32 spellId = NULL) const;
    ...
    uint32 CalcStackAmount(Unit const* caster = NULL) const;
    ...
    bool ShouldTriggerGiftOfTheSerpent() const;
