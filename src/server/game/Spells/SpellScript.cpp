...
SpellCastResult SpellScript::CheckCastHandler::Call(SpellScript* spellScript)
{
    ...
    MEASURE_SPELL((spellScript->*_checkCastHandlerScript), "SpellScript::CheckCastHandler", 1, result);
...
{
    MEASURE_SPELL_VOID((spellScript->*pObjectAreaTargetSelectHandlerScript), "SpellScript::ObjectAreaTargetSelectHandler", 1, targets);
}
...
{
    MEASURE_SPELL_VOID((spellScript->*pObjectTargetSelectHandlerScript), "SpellScript::ObjectTargetSelectHandler", 1, target);
}
...
int32 SpellScript::GetEffectValue()
{
    if (!IsInEffectHook())
    {
        sLog->outError(LOG_FILTER_TSCR, "Script: `%s` Spell: `%u`: function SpellScript::GetEffectValue was called, but function has no effect in current hook!", m_scriptName->c_str(), m_scriptSpellId);
        return 0;
    }
...
void SpellScript::SetEffectValue(int32 value)
{
    if (!IsInEffectHook())
    {
        sLog->outError(LOG_FILTER_TSCR, "TSCR: Script: `%s` Spell: `%u`: function SpellScript::SetEffectValue was called, but function has no effect in current hook!", m_scriptName->c_str(), m_scriptSpellId);
        return;
    }
...
bool AuraScript::_Validate(SpellInfo const* entry)
{
    ...
    for (std::list<EffectAbsorbHealHandler>::iterator itr = OnEffectAbsorbHeal.begin(); itr != OnEffectAbsorbHeal.end();  ++itr)
        if (!(*itr).GetAffectedEffectsMask(entry))
...
{
    bool result;
    MEASURE_AURA((auraScript->*pHandlerScript), "AuraScript::CheckAreaTargetHandler", 1, result, _target);
    return result;
}
...
{
    MEASURE_AURA_VOID((auraScript->*pEffectHandlerScript), "AuraScript::EffectPeriodicHandler", 2, _aurEff);
}
...
{
    MEASURE_AURA_VOID((auraScript->*pEffectHandlerScript), "AuraScript::EffectUpdatePeriodicHandler", 2, aurEff);
}
...
bool AuraScript::EffectCalcAmountHandler::Call(AuraScript* auraScript, AuraEffect const* aurEff, int32& amount, bool& canBeRecalculated)
{
    bool result;
...
{
    MEASURE_AURA_VOID((auraScript->*pEffectHandlerScript), "AuraScript::EffectCalcPeriodicHandler", 2, aurEff, isPeriodic, periodicTimer);
}
...
void AuraScript::EffectAbsorbHealHandler::Call(AuraScript* auraScript, AuraEffect* aurEff, int32 &remainingHeal, uint32& absorbAmount)
{
    MEASURE_AURA_VOID((auraScript->*pEffectHandlerScript), "AuraScript::EffectAbsorbHealHandler", 2, aurEff, remainingHeal, absorbAmount);
}
...
{
    MEASURE_AURA_VOID((auraScript->*pEffectHandlerScript), "AuraScript::EffectAbsorbHandler", 2, aurEff, dmgInfo, absorbAmount);
}
...
{
    MEASURE_AURA_VOID((auraScript->*pEffectHandlerScript), "AuraScript::EffectManaShieldHandler", 2, aurEff, dmgInfo, absorbAmount);
}
...
{
    bool result;
    MEASURE_AURA((auraScript->*_HandlerScript), "AuraScript::CheckProcHandler", 1, result, eventInfo);
    return result;
}
