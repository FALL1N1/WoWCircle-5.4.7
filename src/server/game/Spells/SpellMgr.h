...
enum ProcFlags
{
    ...
    NEGATIVE_PROC_FLAG_MASK = PROC_FLAG_KILLED | PROC_FLAG_KILL
                              ...
                              | PROC_FLAG_DONE_SPELL_NONE_DMG_CLASS_NEG | PROC_FLAG_TAKEN_SPELL_NONE_DMG_CLASS_NEG
                              | PROC_FLAG_DONE_SPELL_MAGIC_DMG_CLASS_NEG | PROC_FLAG_TAKEN_SPELL_MAGIC_DMG_CLASS_NEG
                              | PROC_FLAG_TAKEN_DAMAGE | PROC_FLAG_DONE_MAINHAND_ATTACK
                              | PROC_FLAG_DONE_OFFHAND_ATTACK | PROC_FLAG_DEATH
};
...
class SpellMgr
{
    ...
    public:
        ...
        bool IsSpellProcEventCanTriggeredBy(SpellProcEventEntry const* spellProcEvent, uint32 EventProcFlag, SpellInfo const* procSpell, SpellInfo const* auraSpell, uint32 procFlags, uint32 procExtra, bool active);
        ...
        std::set<uint32> const& GetSpellClassList(uint8 ClassID) const { return mSpellClassInfo[ClassID]; }
        ...
        bool IsInSpellComsumableItems(uint32 entry) const;

        bool IsInSpellAurasNotSave(uint32 spellId) const;
    ...
    private:
        ...
        std::set<uint32>           mSpellConsumableItems;
        std::set<uint32>           mSpellAurasNotSave;
