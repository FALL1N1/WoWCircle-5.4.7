...
void WorldSession::HandleRequestRatedBgStats(WorldPacket& /*p_RecvData*/)
{
    WorldPacket l_Data(Opcodes::SMSG_BATTLEFIELD_RATED_INFO, (8 * 4) * ArenaSlots::MAX_PVP_SLOT);

    for (uint8 l_I = 0; l_I < ArenaSlots::MAX_PVP_SLOT; l_I++)
    {
        l_Data << uint32(_player->GetBestRatingOfSeason(l_I));  ///< BestSeasonRating
        l_Data << uint32(_player->GetWeekGames(l_I));           ///< WeeklyPlayed
        l_Data << uint32(_player->GetBestRatingOfWeek(l_I));    ///< BestWeeklyRating
        l_Data << uint32(_player->GetSeasonGames(l_I));         ///< SeasonPlayed
        l_Data << uint32(_player->GetArenaPersonalRating(l_I)); ///< PersonalRating

        /// SeasonWon or Ranking
        l_Data << uint32(0);

        l_Data << uint32(_player->GetWeekWins(l_I));            ///< WeeklyWon
