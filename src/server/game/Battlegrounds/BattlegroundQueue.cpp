...
bool BattlegroundQueue::SelectionPool::AddGroup(GroupQueueInfo* ginfo, uint32 desiredCount, uint32 mapId)
{
    uint32 mapId1 = ginfo->disabledBGs.empty() ? 0xFFFFFFFF : ginfo->disabledBGs[0];
    uint32 mapId2 = ginfo->disabledBGs.empty() ? 0xFFFFFFFF : ginfo->disabledBGs[1];

    bool check = mapId == 0 || (mapId != mapId1 && mapId != mapId2);
    //if group is larger than desired count - don't allow to add it to pool
    if (check && !ginfo->IsInvitedToBGInstanceGUID && desiredCount >= PlayerCount + ginfo->Players.size())
    {
...
GroupQueueInfo* BattlegroundQueue::AddGroup(Player* leader, Group* grp, BattlegroundTypeId BgTypeId, PvPDifficultyEntry const*  bracketEntry, uint8 ArenaType, bool isRated, bool isPremade, uint32 ArenaRating, uint32 MatchmakerRating, uint32 const* disabledBGs)
{
    ...
    if (disabledBGs)
    {
        ginfo->disabledBGs.push_back(disabledBGs[0]);
        ginfo->disabledBGs.push_back(disabledBGs[1]);
    }
...
void BattlegroundQueue::FillPlayersToBG(Battleground* bg, BattlegroundBracketId bracket_id)
{
    ...
    auto mapId = bg->IsRandom() ? bg->GetMapId() : 0;
    ...
    uint32 aliIndex = 0;
    for (; aliIndex < aliCount && m_SelectionPools[BG_TEAM_ALLIANCE].AddGroup((*Ali_itr), aliFree, mapId); aliIndex++)
        ++Ali_itr;
    //the same thing for horde
    GroupsQueueType::const_iterator Horde_itr = m_QueuedGroups[bracket_id][BG_QUEUE_NORMAL_HORDE].begin();
    uint32 hordeCount = m_QueuedGroups[bracket_id][BG_QUEUE_NORMAL_HORDE].size();
    uint32 hordeIndex = 0;
    for (; hordeIndex < hordeCount && m_SelectionPools[BG_TEAM_HORDE].AddGroup((*Horde_itr), hordeFree, mapId); hordeIndex++)
        ++Horde_itr;
    ...
    */
    ...
    while (abs(diffAli - diffHorde) > 1 && (m_SelectionPools[BG_TEAM_HORDE].GetPlayerCount() > 0 || m_SelectionPools[BG_TEAM_ALLIANCE].GetPlayerCount() > 0))
    {
        ...
        if (diffAli < diffHorde)
        {
            ...
            {
                for (; aliIndex < aliCount && m_SelectionPools[BG_TEAM_ALLIANCE].AddGroup((*Ali_itr), (aliFree >= diffHorde) ? aliFree - diffHorde : 0, mapId); aliIndex++)
                    ++Ali_itr;
            }
        ...
        else
        {
            ...
            {
                for (; hordeIndex < hordeCount && m_SelectionPools[BG_TEAM_HORDE].AddGroup((*Horde_itr), (hordeFree >= diffAli) ? hordeFree - diffAli : 0, mapId); hordeIndex++)
                    ++Horde_itr;
            }
...
bool BattlegroundQueue::SelectArenaTeams(uint32 bracket_id, uint32 arenaRating, GroupQueueInfo* & team1, GroupQueueInfo* & team2)
{
    uint32 discardTime = getMSTime() >= sBattlegroundMgr->GetRatingDiscardTimer() ? getMSTime() - sBattlegroundMgr->GetRatingDiscardTimer() : 0;

    GroupQueueInfo* selectedTeam1 = NULL;
    GroupQueueInfo* selectedTeam2 = NULL;

    std::vector<GroupQueueInfo*> lowRatingTeams;
    for (uint8 i = BG_QUEUE_PREMADE_ALLIANCE; i < BG_QUEUE_NORMAL_ALLIANCE; i++)
    {
        for (GroupsQueueType::iterator itr = m_QueuedGroups[bracket_id][i].begin(); itr != m_QueuedGroups[bracket_id][i].end(); ++itr)
        {
    ...
    for (std::vector<GroupQueueInfo*>::const_iterator itr = lowRatingTeams.begin(); itr != lowRatingTeams.end(); ++itr)
    {
        ...
        uint32 selectedRating = selectedTeam->ArenaMatchmakerRating;

        float mmrSteps = floor(float((getMSTime() - selectedTeam->JoinTime) / 60000)); // every 1 minute
        uint32 mmrMaxDiff = mmrSteps * 150;

        uint32 arenaMinRating = (selectedRating <= sBattlegroundMgr->GetMaxRatingDifference()) ? 0 : selectedRating - sBattlegroundMgr->GetMaxRatingDifference();
        uint32 arenaMaxRating = selectedRating + sBattlegroundMgr->GetMaxRatingDifference();
        if (mmrMaxDiff > 0)
        {
        ...
        for (std::vector<GroupQueueInfo*>::const_iterator itr2 = lowRatingTeams.begin(); itr2 != lowRatingTeams.end(); ++itr2)
        {
            ...
            if ((selectedTeam1 && selectedTeam1 == checkTeam) ||
                (selectedTeam2 && selectedTeam2 == checkTeam))
                continue;

            if ((checkTeam->ArenaMatchmakerRating >= arenaMinRating &&
                checkTeam->ArenaMatchmakerRating <= arenaMaxRating) ||
                checkTeam->JoinTime < discardTime)
            {
        ...
        {
            sBattlegroundMgr->ScheduleQueueUpdate(selectedTeam1->ArenaMatchmakerRating, selectedTeam1->ArenaType, m_queueTypeId, selectedTeam1->BgTypeId, (BattlegroundBracketId)bracket_id);
            break;
        }
    }
...
*/
void BattlegroundQueue::BattlegroundQueueUpdate(uint32 /*diff*/, BattlegroundTypeId bgTypeId, BattlegroundBracketId bracket_id, uint8 arenaType, bool isRated, uint32 arenaRating, uint32 const* disabledBGs)
{
    ...
    if (bg_template->isBattleground() && !bg_template->IsRatedBG())
    {
        ...
        if (CheckPremadeMatch(bracket_id, MinPlayersPerTeam, MaxPlayersPerTeam))
        {
            ...
            Battleground* bg2 = sBattlegroundMgr->CreateNewBattleground(bgTypeId, bracketEntry, 0, false, disabledBGs);
            if (!bg2)
            {
    ...
    if (!isRated && !bg_template->IsRatedBG())
    {
            ...
            || (bg_template->isArena() && CheckSkirmishForSameFaction(bracket_id, MinPlayersPerTeam)))
        {
            ...
            Battleground* bg2 = sBattlegroundMgr->CreateNewBattleground(bgTypeId, bracketEntry, arenaType, false, disabledBGs);
            if (!bg2)
            {
                sLog->outError(LOG_FILTER_BATTLEGROUND, "BattlegroundQueue::Update - Cannot create battleground: %u", bgTypeId);
                return;
    ...
    else if (bg_template->isArena())
    {
        ...
        GroupQueueInfo* aTeam = NULL;
        GroupQueueInfo* hTeam =  NULL;
        if (!SelectArenaTeams(bracket_id, arenaRating, aTeam, hTeam))
            return;

        if (!aTeam || !hTeam)
            return;

        Battleground* arena = sBattlegroundMgr->CreateNewBattleground(bgTypeId, bracketEntry, arenaType, true, disabledBGs);
        if (!arena)
        {
    ...
    else if (bg_template->IsRatedBG())
    {
        ...
        if (found == 2)
        {
            ...
            Battleground* rated_bg = sBattlegroundMgr->CreateNewBattleground(bgTypeId, bracketEntry, 0, false, disabledBGs);
            if (!rated_bg)
            {
