...
void BattlegroundIC::DoAction(uint32 action, uint64 var)
{
    ...
    player->m_movementInfo.transport.pos.m_positionX = ((player->GetTeamId() == TEAM_ALLIANCE) ? allianceShipPos : hordeShipPos).GetPositionX();
    player->m_movementInfo.transport.pos.m_positionY = ((player->GetTeamId() == TEAM_ALLIANCE) ? allianceShipPos : hordeShipPos).GetPositionY();
    player->m_movementInfo.transport.pos.m_positionZ = ((player->GetTeamId() == TEAM_ALLIANCE) ? allianceShipPos : hordeShipPos).GetPositionZ();
    player->m_movementInfo.transport.pos.m_orientation = ((player->GetTeamId() == TEAM_ALLIANCE) ? allianceShipPos : hordeShipPos).GetOrientation();
...
{
    Player::AppendWorldState(data, uint32(BG_IC_ALLIANCE_RENFORT_SET), uint32(1));
    Player::AppendWorldState(data, uint32(BG_IC_HORDE_RENFORT_SET), uint32(1));
    Player::AppendWorldState(data, uint32(BG_IC_ALLIANCE_RENFORT), uint32(factionReinforcements[TEAM_ALLIANCE]));
    Player::AppendWorldState(data, uint32(BG_IC_HORDE_RENFORT), uint32(factionReinforcements[TEAM_HORDE]));
    ...
    for (uint8 i = 0; i < MAX_FORTRESS_GATES_SPAWNS; i++)
    {
        ...
        Player::AppendWorldState(data, uint32(uws), uint32(1));
    }

    for (uint8 i = 0; i < MAX_NODE_TYPES; i++)
        Player::AppendWorldState(data, uint32(nodePoint[i].worldStates[nodePoint[i].nodeState]), uint32(1));
}
