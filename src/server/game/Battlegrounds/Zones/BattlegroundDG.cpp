...
void BattlegroundDG::StartingEventCloseDoors()
{
    ...
    // Spawn Buffs
    for (uint32 l_Iter = 0; l_Iter < MAX_BUFFS_DG; l_Iter++)
        ...
        if (Creature* l_CapturePoint = GetBgMap()->GetCreature(BgCreatures[BG_DG_OBJECT_CAPT_POINT_START + l_Iter]))
        {
            for (BattlegroundPlayerMap::iterator l_Itr = m_Players.begin(); l_Itr != m_Players.end(); ++l_Itr)
            {
                if (Player* l_Player = ObjectAccessor::FindPlayer(l_Itr->first))
                    l_CapturePoint->SendUpdateToPlayer(l_Player); // does it needed?
            }
        }
    }
}
...
{
    Player::AppendWorldState(p_Data, uint32(WORLDSTATE_DG_CART_STATE_ALLIANCE), uint32((_flagState[TEAM_ALLIANCE] == BG_DG_CART_STATE_ON_PLAYER) ? 2 : 1));
    Player::AppendWorldState(p_Data, uint32(WORLDSTATE_DG_CART_STATE_HORDE), uint32((_flagState[TEAM_HORDE] == BG_DG_CART_STATE_ON_PLAYER) ? 2 : 1));
    ...
    for (uint8 l_Node = 0; l_Node < BG_DG_ALL_NODES_COUNT; ++l_Node)
    {
        for (uint8 l_Status = BG_DG_NODE_TYPE_NEUTRAL; l_Status <= BG_DG_NODE_STATUS_HORDE_OCCUPIED; l_Status++)
        {
            if (m_Nodes[l_Node] == l_Status)
                Player::AppendWorldState(p_Data, uint32(BG_DG_NodesWs[l_Node][l_Status].m_WorldState), uint32(BG_DG_NodesWs[l_Node][l_Status].m_SentValueToShow));
            else if (l_Status >= BG_DG_NODE_TYPE_OCCUPIED)
            {
                ...
                if (m_Nodes[l_Node] < BG_DG_NODE_TYPE_OCCUPIED)
                    Player::AppendWorldState(p_Data, uint32(BG_DG_NodesWs[l_Node][l_Status].m_WorldState), uint32(0));
            }
            else
                Player::AppendWorldState(p_Data, uint32(BG_DG_NodesWs[l_Node][l_Status].m_WorldState), uint32(0));
        }
    ...
    Player::AppendWorldState(p_Data, uint32(WORLDSTATE_DG_OCCUPIED_BASES_ALLIANCE), uint32(l_Ally));
    Player::AppendWorldState(p_Data, uint32(WORLDSTATE_DG_OCCUPIED_BASES_HORDE), uint32(l_Horde));
    
    Player::AppendWorldState(p_Data, uint32(WORLDSTATE_DG_SCORE_ALLIANCE), uint32(m_TeamScores[TEAM_ALLIANCE]));
    Player::AppendWorldState(p_Data, uint32(WORLDSTATE_DG_SCORE_HORDE), uint32(m_TeamScores[TEAM_HORDE]));
}
...
void BattlegroundDG::_UpdateTeamScore(int p_Team, int32 p_Value)
{
    ...
    {
        SendMessageToAll(p_Team == TEAM_ALLIANCE ? LANG_BG_DG_H_NEAR_VICTORY : LANG_BG_DG_A_NEAR_VICTORY, CHAT_MSG_BG_SYSTEM_NEUTRAL);
...
void BattlegroundDG::_postUpdateImpl_Flags(uint32 p_Diff)
{
    ...
    for (int l_Node = 0; l_Node < BG_DG_ALL_NODES_COUNT; ++l_Node)
    {
        ...
        for (int l_Team = 0; l_Team < 2; ++l_Team)
        {
    ...
    for (int l_Team = 0; l_Team < 2; ++l_Team)
    {
        ...
        if (m_lastTick[l_Team] > BG_DG_TickIntervals[l_Points])
        {
            ...
            {
                RewardHonorToTeam(GetBonusHonorFromKill(1), (l_Team == TEAM_ALLIANCE) ? ALLIANCE : HORDE);
...
void BattlegroundDG::EventPlayerCapturedFlag(Player* p_Player)
{
    ...
    RewardHonorToTeam(GetBonusHonorFromKill(2), p_Player->GetBGTeam());
...
void BattlegroundDG::EndBattleground(uint32 p_Winner)
{
    ...
    uint32 m_HonorWinKills = (isBGWeekend) ? 3 : 1;
    uint32 m_HonorEndKills = (isBGWeekend) ? 4 : 2;

    auto winner = p_Winner == TEAM_ALLIANCE ? ALLIANCE : HORDE;
...
class spell_mine_cart : public SpellScriptLoader
{
    public:
        ...
        class spell_mine_cart_AuraScript : public AuraScript
        {
            ...
            void Register()
            {
                OnEffectRemove += AuraEffectRemoveFn(spell_mine_cart_AuraScript::HandleOnRemove, EFFECT_3, SPELL_AURA_TRIGGER_AT_APPLY, AURA_EFFECT_HANDLE_REAL);
            }
        };
