...
void BattlegroundWS::RemoveAssaultSpells()
{
    for (uint8 index = 0; index < 2; ++index)
    {
        if (m_FlagKeepers[index] == 0LL)
            continue;

        if (Player* player = ObjectAccessor::FindPlayer(m_FlagKeepers[index]))
            RemoveAssaultSpellsFromPlayer(player);
    }
}
   
void BattlegroundWS::RemoveAssaultSpellsFromPlayer(Player* player)
{
    if (!player)
        return;
...
void BattlegroundWS::ApplyAssaultAura()
{
    if (m_state <= AssaultState::Countdown)
        return;
    ...
    for (uint8 index = 0; index < 2; ++index)
    {
        ...
        if (Player* player = ObjectAccessor::FindPlayer(m_FlagKeepers[index]))
        {
            ApplyAssaultAuraForPlayer(player);
        }
    }
}

void BattlegroundWS::ApplyAssaultAuraForPlayer(Player* player)
{
    if (!player)
        return;
    ...
    switch (m_state)
    {
        case AssaultState::Focused:
            if (auto aura = player->AddAura(WS_SPELL_FOCUSED_ASSAULT, player))
            {
                uint8 stacks = (((m_asTimer - FOCUSED_ASSAULT_TIMER) / IN_MILLISECONDS) / MINUTE) + 1;
                aura->SetStackAmount(stacks);
            }
            break;
        case AssaultState::Brutal:
            player->RemoveAurasDueToSpell(WS_SPELL_FOCUSED_ASSAULT);
            if (auto aura = player->AddAura(WS_SPELL_BRUTAL_ASSAULT, player))
            {
                uint8 stacks = (((m_asTimer - BRUTAL_ASSAULT_TIMER) / IN_MILLISECONDS) / MINUTE) + 1;
                aura->SetStackAmount(stacks);
            }
            break;
    }
...
void BattlegroundWS::UpdateAssaultAura()
{
    if (m_state <= AssaultState::Countdown)
        return;
    ...
    for (uint8 index = 0; index < 2; ++index)
    {
        ...
        if (Player* player = ObjectAccessor::FindPlayer(m_FlagKeepers[index]))
        {
            ...
            {
            case AssaultState::Focused:
                if (auto aura = player->GetAura(WS_SPELL_FOCUSED_ASSAULT))
                {
                    uint8 stacks = (((m_asTimer - FOCUSED_ASSAULT_TIMER) / IN_MILLISECONDS) / MINUTE) + 1;
                    if (stacks != aura->GetStackAmount())
                        aura->SetStackAmount(stacks);
                }
                break;
            case AssaultState::Brutal:
                if (auto aura = player->GetAura(WS_SPELL_BRUTAL_ASSAULT))
                {
                    uint8 stacks = (((m_asTimer - BRUTAL_ASSAULT_TIMER) / IN_MILLISECONDS) / MINUTE) + 1;
                    if (stacks != aura->GetStackAmount())
                        aura->SetStackAmount(stacks);
...
void BattlegroundWS::SetAssaultState(AssaultState newState)
{
    if (m_state == newState)
        return;
    ...
    switch (m_state)
    {
        case AssaultState::OnBase:
        {
            m_asTimer = 0;
            RemoveAssaultSpells();
            break;
        }
        case AssaultState::Countdown:
            break;
        case AssaultState::Brutal:
        case AssaultState::Focused:
            ApplyAssaultAura();
...
void BattlegroundWS::HandleAssault(uint32 diff)
{
    bool bothOnBaseOrCaptured = (_flagState[BG_TEAM_HORDE] == BG_WS_FLAG_STATE_ON_BASE || _flagState[BG_TEAM_HORDE] == BG_WS_FLAG_STATE_WAIT_RESPAWN)
        || (_flagState[BG_TEAM_ALLIANCE] == BG_WS_FLAG_STATE_ON_BASE || _flagState[BG_TEAM_ALLIANCE] == BG_WS_FLAG_STATE_WAIT_RESPAWN);

    if (bothOnBaseOrCaptured)
    {
        SetAssaultState(AssaultState::OnBase);
        return;
    }
    else if (m_state == AssaultState::OnBase && !bothOnBaseOrCaptured)
        SetAssaultState(AssaultState::Countdown);

    if (m_state == AssaultState::OnBase)
        return;

    m_asTimer += diff;
    if (m_asTimer >= FOCUSED_ASSAULT_TIMER && m_asTimer < BRUTAL_ASSAULT_TIMER)
    {
        SetAssaultState(AssaultState::Focused);
    }
    else if (m_asTimer >= BRUTAL_ASSAULT_TIMER)
    {
        SetAssaultState(AssaultState::Brutal);
    }
...
void BattlegroundWS::EventPlayerCapturedFlag(Player* Source)
{
    ...
    RemoveAssaultSpellsFromPlayer(Source);
...
void BattlegroundWS::EventPlayerClickedOnFlag(Player* Source, GameObject* target_obj)
{
    ...
    if (GetFlagState(ALLIANCE) == BG_WS_FLAG_STATE_ON_GROUND && Source->IsWithinDistInMap(target_obj, 10) && target_obj->GetGOInfo()->entry == BG_OBJECT_A_FLAG_GROUND_WS_ENTRY)
    {
        ...
        else
        {
            ...
            _flagState[BG_TEAM_ALLIANCE] = BG_WS_FLAG_STATE_ON_PLAYER;
            UpdateFlagState(HORDE, BG_WS_FLAG_STATE_ON_PLAYER);
            ApplyAssaultAuraForPlayer(Source);
            UpdateWorldState(BG_WS_FLAG_UNK_ALLIANCE, 1);
        }
...
{
    Player::AppendWorldState(data, uint32(BG_WS_FLAG_CAPTURES_ALLIANCE), uint32(GetTeamScore(ALLIANCE)));
    Player::AppendWorldState(data, uint32(BG_WS_FLAG_CAPTURES_HORDE), uint32(GetTeamScore(HORDE)));

    if (_flagState[BG_TEAM_ALLIANCE] == BG_WS_FLAG_STATE_ON_GROUND)
        Player::AppendWorldState(data, uint32(BG_WS_FLAG_UNK_ALLIANCE), uint32(-1));
    else if (_flagState[BG_TEAM_ALLIANCE] == BG_WS_FLAG_STATE_ON_PLAYER)
        Player::AppendWorldState(data, uint32(BG_WS_FLAG_UNK_ALLIANCE), uint32(1));
    else
        Player::AppendWorldState(data, uint32(BG_WS_FLAG_UNK_ALLIANCE), uint32(0));

    if (_flagState[BG_TEAM_HORDE] == BG_WS_FLAG_STATE_ON_GROUND)
        Player::AppendWorldState(data, uint32(BG_WS_FLAG_UNK_HORDE), uint32(-1));
    else if (_flagState[BG_TEAM_HORDE] == BG_WS_FLAG_STATE_ON_PLAYER)
        Player::AppendWorldState(data, uint32(BG_WS_FLAG_UNK_HORDE), uint32(1));
    else
        Player::AppendWorldState(data, uint32(BG_WS_FLAG_UNK_HORDE), uint32(0));

    Player::AppendWorldState(data, uint32(BG_WS_FLAG_CAPTURES_MAX), uint32(BG_WS_MAX_TEAM_SCORE));

    if (GetStatus() == STATUS_IN_PROGRESS)
    {
        Player::AppendWorldState(data, uint32(BG_WS_STATE_TIMER_ACTIVE), uint32(1));
        Player::AppendWorldState(data, uint32(BG_WS_STATE_TIMER), uint32(25 - _minutesElapsed));
    }
    else
        Player::AppendWorldState(data, uint32(BG_WS_STATE_TIMER_ACTIVE), uint32(0));

    if (_flagState[BG_TEAM_HORDE] == BG_WS_FLAG_STATE_ON_PLAYER)
        Player::AppendWorldState(data, uint32(BG_WS_FLAG_STATE_ALLIANCE), uint32(2));
    else
        Player::AppendWorldState(data, uint32(BG_WS_FLAG_STATE_ALLIANCE), uint32(1));

    if (_flagState[BG_TEAM_ALLIANCE] == BG_WS_FLAG_STATE_ON_PLAYER)
        Player::AppendWorldState(data, uint32(BG_WS_FLAG_STATE_HORDE), uint32(2));
    else
        Player::AppendWorldState(data, uint32(BG_WS_FLAG_STATE_HORDE), uint32(1));
