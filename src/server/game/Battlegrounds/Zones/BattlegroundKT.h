...
enum BG_KT_Objects
{
    ...
    BG_KT_OBJECT_BUFF_NORTH     = 6,
    BG_KT_OBJECT_BUFF_SOUTH     = 7,
    BG_KT_OBJECT_MAX            = 8
};
...
#define MAX_BUFFS_KT 2

const float BG_KT_BuffPositions[MAX_BUFFS_KT][4] =
{
...
const uint32 BG_KT_ORB_OWNER_DIED_POINTS = 20;
...
class BattlegroundKT : public Battleground
{
    ...
    public:
        ...
        bool HaveOrb(uint64 const& guid)
        {
            return GetOrbIndex(guid) != MAX_ORBS;
        }

        uint8 GetOrbIndex(uint64 const& guid)
        {
            for (uint8 i = 0; i < MAX_ORBS; ++i)
                if (m_OrbKeepers[i] == guid)
                    return i;
            return MAX_ORBS;
        }

        void SetOrb(uint64 const& guid, uint8 const index)
        {
            m_OrbKeepers[index] = guid;
            m_UpdatePointsTimer[index] = guid > 0LL ? 0 : -1;
        }

        void TakeOrb(Player* player, uint8 const index);
        void ReleaseOrb(Player* player, uint8 const index);
        ...
        void AccumulateScoreByValue(uint32 team, uint32 value);
    private:
        ...
        uint32 m_EndTimer;
        int32 m_UpdatePointsTimer[MAX_ORBS];
