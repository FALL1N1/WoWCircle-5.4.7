...
#define FOCUSED_ASSAULT_TIMER (3 * MINUTE*IN_MILLISECONDS)
#define BRUTAL_ASSAULT_TIMER ((10 * MINUTE*IN_MILLISECONDS) + FOCUSED_ASSAULT_TIMER)
...
class BattlegroundWS : public Battleground
{
    ...
    private:
        ...
        void HandleAssault(uint32 diff);

        void SetAssaultState(AssaultState newState);

        void RemoveAssaultSpells();
        void RemoveAssaultSpellsFromPlayer(Player* player);
        
        void ApplyAssaultAura();
        void ApplyAssaultAuraForPlayer(Player* player);

        void UpdateAssaultAura();

        AssaultState m_state;
        uint32 m_asTimer;
};
#endif
