...
void BattlegroundSM::Reset()
{
    ...
    {
        for (uint8 k_Iter = 0; k_Iter < SM_MAX_PATHS; ++k_Iter)
            m_PathDone[l_Iter][k_Iter] = false;
    }
...
void BattlegroundSM::AddPoints(uint32 p_Team, uint32 p_Points)
{
    ...
    {
        RewardHonorToTeam(GetBonusHonorFromKill(2), p_Team);
...
{
    Player::AppendWorldState(p_Data, uint32(SM_MINE_CARTS_DISPLAY), uint32(DISPLAY_WORLDSTATE));
    Player::AppendWorldState(p_Data, uint32(SM_ALLIANCE_RESOURCES), uint32(m_TeamScores[TEAM_ALLIANCE]));
    Player::AppendWorldState(p_Data, uint32(SM_HORDE_RESOURCES), uint32(m_TeamScores[TEAM_HORDE]));

    Player::AppendWorldState(p_Data, uint32(SM_MINE_CART_1), uint32(DISPLAY_WORLDSTATE));
    Player::AppendWorldState(p_Data, uint32(SM_MINE_CART_2), uint32(DISPLAY_WORLDSTATE));
    Player::AppendWorldState(p_Data, uint32(SM_DISPLAY_ALLIANCE_RESSOURCES), uint32(DISPLAY_WORLDSTATE));
    Player::AppendWorldState(p_Data, uint32(SM_MINE_CART_3), uint32(DISPLAY_WORLDSTATE));
    Player::AppendWorldState(p_Data, uint32(SM_DISPLAY_HORDE_RESSOURCES), uint32(DISPLAY_WORLDSTATE));

    Player::AppendWorldState(p_Data, uint32(SM_DISPLAY_PROGRESS_BAR), uint32(BG_SM_PROGRESS_BAR_DONT_SHOW)); // This shows the mine cart control bar
    Player::AppendWorldState(p_Data, uint32(SM_PROGRESS_BAR_STATUS), uint32(BG_SM_PROGRESS_BAR_NEUTRAL)); // Neutral
    Player::AppendWorldState(p_Data, uint32(SM_UNK), uint32(0));
}
