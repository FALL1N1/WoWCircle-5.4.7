...
{
    {
        ...
        QueryResult result = CharacterDatabase.Query("SELECT g.leaderGuid, g.lootMethod, g.looterGuid, g.lootThreshold, g.icon1, g.icon2, g.icon3, g.icon4, g.icon5, g.icon6"
            ...
            ", g.icon7, g.icon8, g.groupType, g.difficulty, g.raiddifficulty, g.guid, lfg.dungeon, lfg.state, c.name FROM groups g LEFT JOIN lfg_data lfg ON lfg.guid = g.guid LEFT JOIN characters c ON c.guid = g.leaderGuid ORDER BY g.guid ASC");
        if (!result)
        {
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, "Loading Group members...");
    {
        ...
        //                                                    0        1           2                   3         4        5
        QueryResult result = CharacterDatabase.Query("SELECT g.guid, g.memberGuid, g.memberFlags, g.subgroup, g.roles, c.name FROM group_member g LEFT JOIN characters c ON c.guid = g.memberGuid ORDER BY guid");
        if (!result)
        {
        ...
        do
        {
            ...
            if (group)
                group->LoadMemberFromDB(fields[1].GetUInt32(), fields[2].GetUInt8(), fields[3].GetUInt8(), fields[4].GetUInt8(), fields[5].GetString());
            else
