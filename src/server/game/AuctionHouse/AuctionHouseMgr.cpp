...
{
    uint64 owner_guid = MAKE_NEW_GUID(auction->owner, 0, HighGuid::Player);
...
void AuctionHouseMgr::SendAuctionSuccessfulMail(AuctionEntry* auction, SQLTransaction& trans)
{
    ...
    if (owner || owner_accId)
    {
        uint64 profit = auction->bid + uint64(auction->deposit) - auction->GetAuctionCut();
...
bool AuctionEntry::BuildAuctionInfo(WorldPacket& data) const
{
    ...
    data << uint32(0);                                              // Unk
    data << uint64(MAKE_NEW_GUID(owner, 0, HighGuid::Player));       // Auction->owner
    ...
    data << uint64(MAKE_NEW_GUID(bidder, 0, HighGuid::Player));      // auction->bidder current
...
{
    uint64 cut = uint64(CalculatePct(bid, auctionHouseEntry->cutPercent) * sWorld->getRate(RATE_AUCTION_CUT));
    return std::max(cut, (uint64)0);
}
