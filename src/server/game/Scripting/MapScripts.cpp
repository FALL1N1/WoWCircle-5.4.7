        ...
        switch (step.script->command)
        {
            ...
            case SCRIPT_COMMAND_DESPAWN_CREATURE:
            {
                ...
                if (WorldObject* pSummoner = _GetScriptWorldObject(source, true, step.script))
                {
                    ...
                    {
                        auto summoned = pSummoner->FindNearestCreature(step.script->DespawnCreature.CreatureEntry, 100);
                        if (summoned)
                            summoned->DespawnOrUnsummon(step.script->DespawnCreature.DespawnDelay);
                    }
                }
                break;
            }
            ...
            case SCRIPT_COMMAND_ACTIVATE_OBJECT:
                ...
                if (Unit* unit = _GetScriptUnit(source, true, step.script))
                {
                    ...
                    {
                        //sLog->outInfo(LOG_FILTER_SERVER_LOADING, "Script uses gameobject %u %s", pGO->GetEntry(), pGO->GetName());
                        pGO->Use(unit);
                    }
                }
                break;
