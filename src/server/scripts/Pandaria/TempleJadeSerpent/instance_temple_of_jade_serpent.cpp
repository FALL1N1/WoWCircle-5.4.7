...
enum eGameObjects
{
    ...
    LorewalkerEnteringDoor              = 213545,
    ChallengeModeWindDoor               = 239409,
    WiseMariExitDoor                    = 213550
};
...
{
    { eData::DATA_WISE_MARI,            eScenarioDatas::WiseMari    },
    { eData::DATA_LOREWALKER_STONESTEP, eScenarioDatas::Stonestep   },
    { eData::DATA_LIU_FLAMEHEART,       eScenarioDatas::Flameheart  },
    { eData::DATA_SHA_OF_DOUBT,         eScenarioDatas::ShaOfDoubt  },
    { 0,                                0                           }
};

DoorData const g_DoorDatas[] =
{
    { eGameObjects::WiseMariExitDoor,   eData::DATA_WISE_MARI,  DoorType::DOOR_TYPE_PASSAGE,    BoundaryType::BOUNDARY_NONE },
            ...
            void OnGameObjectCreate(GameObject* p_GameObject)
            {
                switch (p_GameObject->GetEntry())
                {
                    case GAMEOBJECT_DOOR_LIU_FLAMEHEART_2:
                        doorLiu_2 = p_GameObject->GetGUID();
                        break;
                    case GAMEOBJECT_DOOR_LOREWALKER_STONSTEP:
                        door_lorewalker = p_GameObject->GetGUID();
                        break;
                    case GAMEOBJECT_DOOR_LIU_FLAMEHEART:
                        doorLiu = p_GameObject->GetGUID();
                        break;
                    case eGameObjects::LorewalkerEnteringDoor:
                        m_EnteringDoor = p_GameObject->GetGUID();
                        p_GameObject->SetGoState(GO_STATE_READY);
                        break;
                    case eGameObjects::ChallengeModeWindDoor:
