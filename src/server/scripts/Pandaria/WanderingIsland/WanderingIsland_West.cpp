...
public:
    ...
    void SummonHiFirepawHelper(Player* summoner, uint32 entry)
    {
        ...
        if (!summon->Create(summoner->GetMap()->GenerateLowGuid<HighGuid::Unit>(), summoner->GetMap(), phase, entry, 0, team, pos.GetPositionX(), pos.GetPositionY(), pos.GetPositionZ(), pos.GetOrientation()))
        {
            delete summon;
            return;
        }
