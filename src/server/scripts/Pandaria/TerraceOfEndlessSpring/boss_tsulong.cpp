...
class boss_tsulong : public CreatureScript
{
    public:
        ...
        struct boss_tsulongAI : public BossAI
        {
            ...
            void EnterEvadeMode()
            {
                if (!pInstance || pInstance->GetBossState(DATA_TSULONG) != IN_PROGRESS)
                    return;
            ...
            void EndOfFight()
            {
                ...
                if (pInstance)
                {
                    ...
                    if (IsHeroic())
                        pInstance->DoCompleteAchievement(ACHIEVEMENT_HEROIC_TSULONG);

                    pInstance->DoRemoveAurasDueToSpellOnPlayers(SPELL_DREAD_SHADOWS);
                    pInstance->DoRemoveAurasDueToSpellOnPlayers(SPELL_DREAD_SHADOWS_DEBUFF);
                    pInstance->DoRemoveAurasDueToSpellOnPlayers(SPELL_NIGHT_PHASE_EFFECT);
                    pInstance->DoRemoveAurasDueToSpellOnPlayers(SPELL_TERRORIZE_PLAYER);
                }
                ...
                uint32 const l_AddEntries[3] = { EMBODIED_TERROR, UNSTABLE_SHA_DUMMY, TINY_TERROR };
                for (uint32 l_Itr = 0; l_Itr < 3; ++l_Itr)
                {
                    std::list<Creature*> l_AddList;
                    GetCreatureListWithEntryInGrid(l_AddList, me, l_AddEntries[l_Itr], 200.f); // TODO: adjuste searche range
            ...
            void DespawnAllTinyTerror()
            {
                ...
                me->GetCreatureListWithEntryInGrid(tinyTerrors, TINY_TERROR, 200.0f); // TODO: adjuste searche range
            ...
            void DespawnAllSunbeams()
            {
                ...
                me->GetCreatureListWithEntryInGrid(sunbeams, SUNBEAM_DUMMY_ENTRY, 200.0f); // TODO: adjuste searche range
                ...
                if (phase == PHASE_DAY)
                {
                    switch (events.ExecuteEvent())
                    {
                        ...
                        case EVENT_UNSTABLE_SHA:
                        {
                            ...
                            me->GetCreatureListWithEntryInGrid(dummys, UNSTABLE_SHA_DUMMY, 200.0f); // TODO: adjuste searche range
                            for (auto itr : dummys)
        ...
        private:
            ...
            void RewardEncounter()
            {
                instance->UpdateEncounterState(ENCOUNTER_CREDIT_CAST_SPELL, SPELL_TSU_LONG, me);
                instance->DoUpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_BE_SPELL_TARGET, SPELL_TSU_LONG, 0, 0, me);
            }
        };
        ...
        struct npc_dark_of_nightAI : public CreatureAI
        {
            ...
            void UpdateAI(uint32 const diff)
            {
                ...
                if (!explode)
                {
                    ...
                    if (sunbeam->GetDistance(me) <= sunbeam->GetFloatValue(OBJECT_FIELD_SCALE_X))
                    {
