...
class boss_ancient_regail : public CreatureScript
{
    public:
                ...
                if (pInstance)
                {
                    ...
                    switch (ProtectorsAlive(pInstance, me))
                    {
                        ...
                        case 0:
                        {
                            ...
                            if (IsHeroic())
                                pInstance->DoCompleteAchievement(ACHIEVEMENT_HEROIC_PROTECTORS);

                            if (killer && killer->GetTypeId() == TYPEID_PLAYER)
                                me->GetMap()->ToInstanceMap()->PermBindAllPlayersForRaid(killer->ToPlayer());
                            else if (killer && killer->GetTypeId() == TYPEID_UNIT && killer->GetOwner() && killer->GetOwner()->ToPlayer())
                                me->GetMap()->ToInstanceMap()->PermBindAllPlayersForRaid(killer->GetOwner()->ToPlayer());
        ...
        private:
            ...
            {
                instance->UpdateEncounterState(ENCOUNTER_CREDIT_CAST_SPELL, SPELL_PROTECTORS_OF_THE_ENDLESS, me);
                instance->DoUpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_BE_SPELL_TARGET, SPELL_PROTECTORS_OF_THE_ENDLESS, 0, 0, me);
            }
