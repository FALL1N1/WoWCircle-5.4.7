...
enum Spells
{
    ...
    SPELL_FOUL_GEYZER           = 143990,
    SPELL_FOUL_GEYZER_MISSILE   = 143992,
    SPELL_FOUL_GEYZER_DMG       = 143993,
...
struct korkron_dark_shaman_AI : public BossAI
{
    korkron_dark_shaman_AI(Creature* creature) : BossAI(creature, DATA_KORKRON_DARK_SHAMANS),
        mountGuid(0), isFirstReset(true), m_TalkHelper(creature), 
...
protected:
    ...
    Creature* GetKardris()
    {
        return instance->instance->GetCreature(instance->GetData64(DATA_WAVEBINDER_KARDRIS));
    }

    Creature* GetHaromm()
    {
        return instance->instance->GetCreature(instance->GetData64(DATA_EARTHBREAKER_HAROMM));
    }
...
private:
    ...
    {
        DespawnCreaturesInArea(NPC_TOXIC_TORNADO, me);
    }
            ...
            void ExecuteEvent(const uint32 eventId) override
            {
                switch (eventId)
                {
                    ...
                    case EVENT_TOXIC_MIST:
                        DoCastAOE(SPELL_TOXIC_MIST);
                        events.ScheduleEvent(EVENT_TOXIC_MIST, TIMER_TOXIC_MIST);
                        break;
                    case EVENT_FOUL_STREAM:
                    {
                        Unit* target = SelectTarget(SELECT_TARGET_RANDOM, -20.0f, 0.0f);
                        if (!target)
                            target = SelectTarget(SELECT_TARGET_RANDOM, 0, 0.0f);
            ...
            void ExecuteEvent(const uint32 eventId) override
            {
                switch (eventId)
                {
                    ...
                    case EVENT_FOUL_GEYSER:
                        DoCastVictim(SPELL_FOUL_GEYZER);
    ...
    public:
        ...
        struct npc_korkron_dark_shaman_wolfAI : public ScriptedAI
        {
            ...
            void EnterCombat(Unit* who)
            {
                ...
                {
                    if (Creature* pHaromm = pInstance->instance->GetCreature(pInstance->GetData64(DATA_EARTHBREAKER_HAROMM)))
                    {
                        if (!pHaromm->isInCombat())
                            DoZoneInCombat(pHaromm);
                    }
        ...
        private:
            ...
            void AttackNearbyPlayer(const uint32 diff)
            {
                if (checkTargetTimer <= diff)
                {
                    ...
                    Unit* target = me->FindNearestPlayer(1.0f);
    ...
    public:
        ...
        struct npc_korkron_dark_shaman_falling_ashAI : public ScriptedAI
        {
            npc_korkron_dark_shaman_falling_ashAI(Creature* creature) : ScriptedAI(creature)
            {
                ...
                me->SetSpeed(MOVE_RUN, FALLING_ASH_SPEED, true);
                me->SetSpeed(MOVE_FLIGHT, FALLING_ASH_SPEED, true);
            }
    ...
    public:
        ...
        class spell_korkron_dark_shaman_foul_stream_SpellScript : public SpellScript
        {
            ...
            void HandleBeforeCast()
            {
                ...
                if (WorldObject* target = GetSpell()->GetUnitTarget())
                {
            ...
            void FilterTargets(std::list<WorldObject*>& targets)
            {
                ...
                Unit* baseTarget = Unit::GetUnit(*caster, baseTargetGuid);
                if (!baseTarget)
                    return;
            ...
            void Register()
            {
                ...
                OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_korkron_dark_shaman_foul_stream_SpellScript::FilterTargets, EFFECT_0, TARGET_UNIT_CONE_ENEMY_129);
                OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_korkron_dark_shaman_foul_stream_SpellScript::FilterTargets, EFFECT_1, TARGET_UNIT_CONE_ENEMY_129);
            }

        private:

            uint64 baseTargetGuid;
    ...
    public:
        ...
        class spell_korkron_dark_shaman_ashen_wall_SpellScript : public SpellScript
        {
            ...
            void HandleHitTarget(SpellEffIndex effIndex)
            {
                ...
                Position centerPos;
                GetHitUnit()->GetPosition(&centerPos);

                for (uint8 i = 0; i < 8; ++i)
                {
                    Position summonPos;
                    GetHitUnit()->GetNearPosition(summonPos, -20.0f + (5.0f * i), GetHitUnit()->GetRelativeAngle(GetCaster()) + (M_PI / 2));
