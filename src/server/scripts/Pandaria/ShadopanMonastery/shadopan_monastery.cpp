...
/// Slain Shado-Pan Defender - 58794
class npc_slain_shado_pan_defender : public CreatureScript
{
    public:
        npc_slain_shado_pan_defender() : CreatureScript("npc_slain_shado_pan_defender") { }

        enum eSpell
        {
            PurifactionRituel = 111690
        };

        struct npc_slain_shado_pan_defenderAI : public ScriptedAI
        {
            npc_slain_shado_pan_defenderAI(Creature* p_Creature) : ScriptedAI(p_Creature) { }

            void Reset() override
            {
                me->SetFlag(EUnitFields::UNIT_FIELD_FLAGS, UnitFlags::UNIT_FLAG_NON_ATTACKABLE);
            }

            void SpellHit(Unit* p_Caster, SpellInfo const* p_SpellInfo) override
            {
                if (p_Caster->GetTypeId() != TypeID::TYPEID_PLAYER)
                    return;

                if (p_SpellInfo == nullptr)
                    return;

                if (p_SpellInfo->Id != eSpell::PurifactionRituel)
                    return;

                if (!me->HasFlag(EUnitFields::UNIT_NPC_FLAGS, NPCFlags::UNIT_NPC_FLAG_SPELLCLICK))
                    return;

                me->RemoveFlag(EUnitFields::UNIT_NPC_FLAGS, NPCFlags::UNIT_NPC_FLAG_SPELLCLICK);

                if (InstanceScript* l_InstanceScript = me->GetInstanceScript())
                    l_InstanceScript->SetData(DATA_DEFENDERS_PURIFIED, 0);
            }
        };

        CreatureAI* GetAI(Creature* p_Creature) const override
        {
            return new npc_slain_shado_pan_defenderAI(p_Creature);
        }
};
...
void AddSC_shadopan_monastery()
{
    ...
    new npc_slain_shado_pan_defender();
