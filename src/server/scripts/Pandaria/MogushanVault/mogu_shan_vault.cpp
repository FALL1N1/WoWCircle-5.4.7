...
void StartNextFight(uint32 p_PreviousAdd, uint32 p_NextAdd, uint32 p_PreviousBoss, uint32 p_NextBoss, uint32 p_Action, uint32 p_NextAction, Creature* me)
{
    ...
    me->GetCreatureListWithEntryInGrid(l_CreatureList, p_PreviousAdd, 200.0f); // TODO: adjuste searche range
    ...
    me->GetCreatureListWithEntryInGrid(l_CreatureList, p_NextAdd, 200.0f); // TODO: adjuste searche range
            ...
            void DoAction(const int32 action)
            {
                switch (action)
                {
                    ...
                    case ACTION_EVENT_WIPE:
                    {
                        ...
                        {
                            std::list<Creature*> mobList;
                            GetCreatureListWithEntryInGrid(mobList, me, eventMobs[i], 200.0f); // TODO: adjuste searche range
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_START_FIRST_COMBAT:
                        {
                            ...
                            me->GetCreatureListWithEntryInGrid(sorcererMoguList, NPC_SORCERER_MOGU, 200.0f); // TODO: adjuste searche range
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_QIANG_START_SECOND_FIGHT:
                        {
                            ...
                            GetCreatureListWithEntryInGrid(l_MountedMoguList, me, NPC_MOUNTED_MOGU, 200.0f); // TODO: adjuste searche range
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_SUBETAI_START_THIRD_COMBAT:
                        {
                            ...
                            GetCreatureListWithEntryInGrid(l_MoguArcherList, me, NPC_MOGU_ARCHER, 200.0f); // TODO: adjuste searche range
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_MENG_START_FOURTH_COMBAT:
                        {
                            ...
                            GetCreatureListWithEntryInGrid(l_KingsGuardsList, me, NPC_KINGSGUARD, 200.0f); // TODO: adjuste searche range
...
class mob_kingsguard : public CreatureScript
{
    public:
        ...
        struct mob_kingsguardAI : public ScriptedAI
        {
            ...
            {
                std::list<Creature*> l_CreatureList;
                me->GetCreatureListWithEntryInGrid(l_CreatureList, me->GetEntry(), 200.0f); // TODO: adjuste searche range
