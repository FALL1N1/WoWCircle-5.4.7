            ...
            void DoAction(int32 const action)
            {
                ...
                switch (action)
                {
                    ...
                    case ACTION_GUARDIAN_DIED:
                    {
                        ...
                        if (IsHeroic())
                            pInstance->DoCompleteAchievement(ACHIEVEMENT_HEROIC_STONE_GUARD);
                    ...
                    case ACTION_POWER_DOWN:
                    {
                        if (Creature* guardian = pInstance->instance->GetCreature(pInstance->GetData64(NPC_AMETHYST)))
                        {
                            ...
                            GetCreatureListWithEntryInGrid(tileList, me, NPC_TILING_CREATURE, 250.0f);
                            ...
                            GetPlayerListInGrid(raid, me, 5000.0f); // TODO: adjuste searche range
            ...
            void UpdateAI(const uint32 diff)
            {
                ...
                switch (events.ExecuteEvent())
                {
                    ...
                    case EVENT_CHECK_PETRIFICATION:
                    {
                        ...
                        GetPlayerListInGrid(playerList, me, 5000.0f); // TODO: adjuste searche range
    ...
    public:
        ...
        class spell_petrification_AuraScript : public AuraScript
        {
            ...
            void HandleTriggerSpell(AuraEffect const* aurEff)
            {
                ...
                std::list<Player*> playerList;
                GetPlayerListInGrid(playerList, caster, 200.0f); // TODO: adjuste searche range
...
/// 116235
class at_amethyst_pool : MS::AreaTriggerEntityScript
{
    public:
        at_amethyst_pool() : MS::AreaTriggerEntityScript("at_amethyst_pool") { }
        ...
        void OnUpdate(AreaTrigger* p_AreaTrigger, uint32 p_Time) override
        {
            ...
            {
                for (Unit* itr : l_TargetList)
                {
                    // Amethyst Pool - Periodic Damage
                    if (itr->GetDistance(p_AreaTrigger) > 3.5f && itr->HasAura(SPELL_AMETHYST_AURA))
                        itr->RemoveAura(SPELL_AMETHYST_AURA);
                    else if (itr->GetDistance(p_AreaTrigger) <= 3.5f && !itr->HasAura(SPELL_AMETHYST_AURA))
                        l_Caster->CastSpell(itr, SPELL_AMETHYST_AURA, true);
                }
            }
        }
        ...
        void OnRemove(AreaTrigger* p_AreaTrigger, bool byExpire) override
        {
            ...
            GetPlayerListInGrid(l_PlayerList, p_AreaTrigger, 3.5f);

            for (Player* l_Player : l_PlayerList)
            {
                if (l_Player->HasAura(SPELL_AMETHYST_AURA))
                    l_Player->RemoveAura(SPELL_AMETHYST_AURA);
            }
        }
};
...
void AddSC_boss_stone_guard()
{
    ...
    new boss_generic_guardian();        ///< 59915 - 60043 - 60047 - 60051
