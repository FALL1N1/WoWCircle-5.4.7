        ...
        struct boss_elegonAI : public BossAI
        {
            ...
            void JustDied(Unit* killer)
            {
                ...
                if (pInstance)
                {
                    ...
                    if (IsHeroic())
                        pInstance->DoCompleteAchievement(ACHIEVEMENT_HEROIC_ELEGON);
        ...
        struct mob_energy_chargeAI : public ScriptedAI
        {
            ...
            void MovementInform(uint32 type, uint32 id)
            {
                switch (id)
                {
                    ...
                    {
                        if (Unit* focus = ObjectAccessor::GetUnit(*me, me->GetUInt64Value(UNIT_FIELD_TARGET)))
                        {
                            Position pos;
        ...
        class spell_destabilizing_energies_AuraScript : public AuraScript
        {
            ...
            void Apply(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
            {
                if (Unit* caster = GetCaster())
                {
                    ...
                    GetPlayerListInGrid(targetList, caster, 200.0f); // TODO: adjuste searche range
...
/// 116546
class at_draw_power : MS::AreaTriggerEntityScript
{
    public:
        at_draw_power() : MS::AreaTriggerEntityScript("at_draw_power") { }
        ...
        void OnUpdate(AreaTrigger* p_AreaTrigger, uint32 p_Time) override
        {
            ...
            for (auto itr : targetList)
            {
                ...
                {
                    if (!itr->HasAura(SPELL_ENERGY_CONDUIT))
                        l_Caster->AddAura(SPELL_ENERGY_CONDUIT, itr);
                }
                else
                    itr->RemoveAurasDueToSpell(SPELL_ENERGY_CONDUIT);
            }
        }
};
