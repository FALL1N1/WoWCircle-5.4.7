    ...
    public:
        ...
        struct boss_fengAI : public BossAI
        {
            ...
            void JustDied(Unit* attacker)
            {
                ...
                if (IsHeroic())
                    pInstance->DoCompleteAchievement(ACHIEVEMENT_HEROIC_FENG);
            ...
            void DoAction(const int32 action)
            {
                switch (action)
                {
                    ...
                    case ACTION_SOUL_REMOVE:
                    {
                        ...
                        GetCreatureListWithEntryInGrid(soulList, me, NPC_SOUL_FRAGMENT, 200.0f); // TODO: adjuste searche range
                ...
                while (uint32 eventID = events.ExecuteEvent())
                {
                    switch (eventID)
                    {
                        case EVENT_SHIELD_CASTSOULS:
                        {
                            ...
                            GetPlayerListInGrid(potenTargets, me, 150.0f); // TODO: adjuste searche range
...
/// 115817
class at_nullification_barrier : MS::AreaTriggerEntityScript
{
    public:
        at_nullification_barrier() : MS::AreaTriggerEntityScript("at_nullification_barrier") { }
        ...
        void OnUpdate(AreaTrigger* p_AreaTrigger, uint32 p_Time) override
        {
            ...
            if (!l_TargetList.empty())
            {
                for (auto itr : l_TargetList)
                    itr->CastSpell(itr, SPELL_FIST_BARRIER, true);
            }
        }
};
