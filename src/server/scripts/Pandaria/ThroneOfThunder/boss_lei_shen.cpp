    ...
    public:
        ...
        struct boss_lei_shenAI : public BossAI
        {
            ...
            void Reset()
            {
                ...
                instance->DoRemoveAuraFromStackOnPlayers(SPELL_DISCHARGED_ENERGY_DMG);
                instance->DoRemoveAuraFromStackOnPlayers(SPELL_CONDUIT_LEVEL_PLAYERS);
            }
            ...
            void EnterCombat(Unit* attacker)
            {
                ...
                ActivateConduitsTargetCheck(true);

                EnableEncounterFramesForAll(true);
            ...
            void EnterEvadeMode()
            {
                EnableEncounterFramesForAll(false);
        ...
        private:
            ...
            void SpawnConduits()
            {
                if (Creature* pConduit = me->SummonCreature(NPC_BOUNCING_BOLT_CONDUIT, conduitPos[0]))
                    bouncingBoltConduitGUID = pConduit->GetGUID();

                if (Creature* pConduit = me->SummonCreature(NPC_OVERCHARGE_CONDUIT, conduitPos[1]))
                    overchargeConduitGUID = pConduit->GetGUID();

                if (Creature* pConduit = me->SummonCreature(NPC_STATIC_SHOCK_CONDUIT, conduitPos[2]))
                    staticShockConduitGUID = pConduit->GetGUID();

                if (Creature* pConduit = me->SummonCreature(NPC_DIFFUSION_CHAIN_CONDUIT, conduitPos[3]))
                    diffusingChainConduitGUID = pConduit->GetGUID();
            }

            void DespawnConduit(Conduits conduit, bool overcharge)
            {
                if (Creature* pConduit = GetConduit(conduit))
                {
                    if (overcharge)
                    {
                        pConduit->AI()->DoAction(ACTION_OVERCHARGE_CONDUIT);
                    }

                    instance->SendEncounterUnit(ENCOUNTER_FRAME_DISENGAGE, pConduit);
                    pConduit->DespawnOrUnsummon(100);
                }

                switch (conduit)
                {
                    case OVERCHARGE_CONDUIT: overchargeConduitGUID = 0; break;
                    case STATIC_SHOCK_CONDUIT: staticShockConduitGUID = 0; break;
                    case DIFFUSION_CHAIN_CONDUIT: diffusingChainConduitGUID = 0; break;
                    case BOUNCING_BOLT_CONDUIT: bouncingBoltConduitGUID = 0; break;
                }
            }

            Creature* GetConduit(Conduits conduit)
            {
                switch (conduit)
                {
                    case BOUNCING_BOLT_CONDUIT: return Creature::GetCreature(*me, bouncingBoltConduitGUID);
                    case OVERCHARGE_CONDUIT: return Creature::GetCreature(*me, overchargeConduitGUID);
                    case STATIC_SHOCK_CONDUIT: return Creature::GetCreature(*me, staticShockConduitGUID);
                    case DIFFUSION_CHAIN_CONDUIT: return Creature::GetCreature(*me, diffusingChainConduitGUID);
                }

                return NULL;
            }

            void ActivateConduitsTargetCheck(bool activate)
            {
                if (Creature* pConduit = GetConduit(BOUNCING_BOLT_CONDUIT))
                {
                    pConduit->AI()->DoAction(activate ? ACTIVATE_COUDUIT_TARGET_CHECK : DEACTIVATE_CONDUIT_TARGET_CHECK);
                }

                if (Creature* pConduit = GetConduit(OVERCHARGE_CONDUIT))
                {
                    pConduit->AI()->DoAction(activate ? ACTIVATE_COUDUIT_TARGET_CHECK : DEACTIVATE_CONDUIT_TARGET_CHECK);
                }

                if (Creature* pConduit = GetConduit(STATIC_SHOCK_CONDUIT))
                {
                    pConduit->AI()->DoAction(activate ? ACTIVATE_COUDUIT_TARGET_CHECK : DEACTIVATE_CONDUIT_TARGET_CHECK);
                }

                if (Creature* pConduit = GetConduit(DIFFUSION_CHAIN_CONDUIT))
                {
                    pConduit->AI()->DoAction(activate ? ACTIVATE_COUDUIT_TARGET_CHECK : DEACTIVATE_CONDUIT_TARGET_CHECK);
                }
            }

            void IncreaseCondiutsLevel()
            {
                if (Creature* pConduit = GetConduit(BOUNCING_BOLT_CONDUIT))
                {
                    pConduit->AI()->DoAction(INCREASE_CONDUIT_LEVEL);
                }
            ...
            void OverchargeRandomConduit()
            {
                Conduits selectedConduit = CONDUIT_NONE;
                int32 selectedConduitPower = 0;

                if (Creature* pConduit = GetConduit(STATIC_SHOCK_CONDUIT))
                {
                    if (pConduit->GetPower(POWER_ENERGY) > selectedConduitPower)
                    {
                        selectedConduit = STATIC_SHOCK_CONDUIT;
                        selectedConduitPower = pConduit->GetPower(POWER_ENERGY);
                    }
                }
                ...
                if (Creature* pConduit = GetConduit(BOUNCING_BOLT_CONDUIT))
                {
                    if (pConduit->GetPower(POWER_ENERGY) > selectedConduitPower)
                    {
                        selectedConduit = BOUNCING_BOLT_CONDUIT;
                        selectedConduitPower = pConduit->GetPower(POWER_ENERGY);
                    }
                }
                ...
                if (Creature* pConduit = GetConduit(OVERCHARGE_CONDUIT))
                {
                    if (pConduit->GetPower(POWER_ENERGY) > selectedConduitPower)
                    {
                        selectedConduit = OVERCHARGE_CONDUIT;
                        selectedConduitPower = pConduit->GetPower(POWER_ENERGY);
                    }
                }
                ...
                if (Creature* pConduit = GetConduit(DIFFUSION_CHAIN_CONDUIT))
                {
                    if (pConduit->GetPower(POWER_ENERGY) > selectedConduitPower)
                    {
                        selectedConduit = DIFFUSION_CHAIN_CONDUIT;
                        selectedConduitPower = pConduit->GetPower(POWER_ENERGY);
                    }
                }

                if (selectedConduit != CONDUIT_NONE)
                {
                    DespawnConduit(selectedConduit, true);
                }
            }

            void DeactivateAllConduits()
            {
                DespawnConduit(STATIC_SHOCK_CONDUIT, false);
                DespawnConduit(BOUNCING_BOLT_CONDUIT, false);
                DespawnConduit(OVERCHARGE_CONDUIT, false);
                DespawnConduit(DIFFUSION_CHAIN_CONDUIT, false);
            }
            ...
            void EnableEncounterFramesForAll(bool enable)
            {
                if (enable)
                {
                    ...
                    if (Creature* pConduit = GetConduit(BOUNCING_BOLT_CONDUIT))
                    {
                        instance->SendEncounterUnit(ENCOUNTER_FRAME_ENGAGE, pConduit);
                    }
            ...
            {
                if (GameObject* pGo = instance->instance->GetGameObject(instance->GetData64(DATA_CHARGING_STATION)))
                {
            ...
            {
                if (GameObject* pFloor = instance->instance->GetGameObject(instance->GetData64(DATA_FLOOR_NORTH)))
                {
                    pFloor->SetGoState(activate ? GO_STATE_ACTIVE : GO_STATE_READY);
                }

                if (GameObject* pFloor = instance->instance->GetGameObject(instance->GetData64(DATA_FLOOR_EAST)))
                {
                    pFloor->SetGoState(activate ? GO_STATE_ACTIVE : GO_STATE_READY);
                }

                if (GameObject* pFloor = instance->instance->GetGameObject(instance->GetData64(DATA_FLOOR_WEST)))
                {
                    pFloor->SetGoState(activate ? GO_STATE_ACTIVE : GO_STATE_READY);
                }

                if (GameObject* pFloor = instance->instance->GetGameObject(instance->GetData64(DATA_FLOOR_SOUTH)))
                {
                    pFloor->SetGoState(activate ? GO_STATE_ACTIVE : GO_STATE_READY);
                }
            }
            ...
            void ActivateRandomWindow()
            {
                ...
                {
                    if (GameObject* pWindow = instance->instance->GetGameObject(windowGuid))
                    {
                        pWindow->SetGoState(GO_STATE_ACTIVE);
                    }
            ...
            {
                if (GameObject* pGo = instance->instance->GetGameObject(instance->GetData64(DATA_WINDOW_NORTH_WEST)))
                    pGo->SetGoState(GO_STATE_READY);

                if (GameObject* pGo = instance->instance->GetGameObject(instance->GetData64(DATA_WINDOW_SOUTH_WEST)))
                    pGo->SetGoState(GO_STATE_READY);

                if (GameObject* pGo = instance->instance->GetGameObject(instance->GetData64(DATA_WINDOW_NORTH_EAST)))
                    pGo->SetGoState(GO_STATE_READY);

                if (GameObject* pGo = instance->instance->GetGameObject(instance->GetData64(DATA_WINDOW_SOUTH_EAST)))
                    pGo->SetGoState(GO_STATE_READY);
            }
            ...
            bool ChangePhase()
            {
                if (phase == PHASE_NONE && me->GetHealthPct() <= 65.0f)
                {
                    ...
                    ActivateConduitsTargetCheck(false);
...
class npc_lei_shen_condiut : public CreatureScript
{
    public:
        ...
        struct npc_lei_shen_condiutAI : public Scripted_NoMovementAI
        {
            ...
            {
                if (type == DATA_COUNDUIT_LEVEL)
                {
                    return conduitLevel;
                }
                else if (type == DATA_CONDUIT_OVERCHARGED)
                {
                    return IsOvercharged ? 1 : 0;
                }
        ...
        private:
            ...
            void ActivateFloor(bool activate)
            {
                ...
                {
                    case NPC_STATIC_SHOCK_CONDUIT:
                        pFloor = pInstance->instance->GetGameObject(pInstance->GetData64(DATA_FLOOR_NORTH));
                        break;
                    case NPC_BOUNCING_BOLT_CONDUIT:
                        pFloor = pInstance->instance->GetGameObject(pInstance->GetData64(DATA_FLOOR_WEST));
                        break;
                    case NPC_OVERCHARGE_CONDUIT:
                        pFloor = pInstance->instance->GetGameObject(pInstance->GetData64(DATA_FLOOR_SOUTH));
                        break;
                    case NPC_DIFFUSION_CHAIN_CONDUIT:
                        pFloor = pInstance->instance->GetGameObject(pInstance->GetData64(DATA_FLOOR_EAST));
                        break;
                }
    ...
    public:
        ...
        struct npc_lei_shen_thunderous_throwAI : public Scripted_NoMovementAI
        {
            ...
            void IsSummonedBy(Unit* owner)
            {
                if (InstanceScript* pInstance = me->GetInstanceScript())
                {
                    if (Creature* pCreature = pInstance->instance->GetCreature(pInstance->GetData64(DATA_LEI_SHEN)))
                    {
        ...
        struct npc_lei_shen_lightning_whipAI : public Scripted_NoMovementAI
        {
            ...
            void IsSummonedBy(Unit* owner)
            {
                if (InstanceScript* pInstance = me->GetInstanceScript())
                {
                    if (Creature* pCreature = pInstance->instance->GetCreature(pInstance->GetData64(DATA_LEI_SHEN)))
                    {
                        pCreature->AI()->SetFloatData(0, pCreature->GetAngle(me));
...
class spell_lei_shen_thunderstruck_dmg : public SpellScriptLoader
{
    public:
        spell_lei_shen_thunderstruck_dmg() : SpellScriptLoader("spell_lei_shen_thunderstruck_dmg") { }

        class spell_lei_shen_thunderstruck_dmg_SpellScript : public SpellScript
        {
            PrepareSpellScript(spell_lei_shen_thunderstruck_dmg_SpellScript);
            ...
            void HandleDamage(SpellEffIndex effIndex)
            {
                ...
                int32 minDamage = 80000;
                int32 calcDamage = GetHitDamage();
                if (WorldLocation const* targetDest = GetSpell()->m_targets.GetDstPos())
                {
                    float distance = GetHitUnit()->GetDistance(targetDest->GetPositionX(), targetDest->GetPositionY(), targetDest->GetPositionZ());
                    int32 reducedDamage = int32(distance * 50000);
                    calcDamage = GetHitDamage() - reducedDamage;
                }
            ...
            void Register()
            {
                OnEffectHitTarget += SpellEffectFn(spell_lei_shen_thunderstruck_dmg_SpellScript::HandleDamage, EFFECT_0, SPELL_EFFECT_SCHOOL_DAMAGE);
            }
        };

        SpellScript* GetSpellScript() const
        {
            return new spell_lei_shen_thunderstruck_dmg_SpellScript();
        }
};
...
class spell_lei_shen_bouncing_bolt_dmg : public SpellScriptLoader
{
    public:
        spell_lei_shen_bouncing_bolt_dmg() : SpellScriptLoader("spell_lei_shen_bouncing_bolt_dmg") { }

        class spell_lei_shen_bouncing_bolt_dmg_SpellScript : public SpellScript
        {
            PrepareSpellScript(spell_lei_shen_bouncing_bolt_dmg_SpellScript);
            ...
            void FilterTargets(std::list<WorldObject*> &targets)
            {
                ...
                if (targets.empty())
                {
                    if (WorldLocation const* targetDest = GetSpell()->m_targets.GetDstPos())
                    {
                        if (Creature* pCreature = GetCaster()->ToCreature())
                        {
                            pCreature->CastSpell(targetDest->GetPositionX(), targetDest->GetPositionY(), targetDest->GetPositionZ(), SPELL_BOUNCING_BOLT_SUMMON_2, true);
                        }
                    }
                }
            }
            ...
            void HandleDamage(SpellEffIndex effIndex)
            {
                ...
                if (Creature* pConduit = GetCaster()->FindNearestCreature(NPC_BOUNCING_BOLT_CONDUIT, 200.0f))
                {
                    uint32 level = pConduit->AI()->GetData(DATA_COUNDUIT_LEVEL);
                    uint32 energy = pConduit->GetPower(POWER_ENERGY);
            ...
            void Register()
            {
                OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_lei_shen_bouncing_bolt_dmg_SpellScript::FilterTargets, EFFECT_0, TARGET_UNIT_DEST_AREA_ENEMY);
                OnEffectHitTarget += SpellEffectFn(spell_lei_shen_bouncing_bolt_dmg_SpellScript::HandleDamage, EFFECT_0, SPELL_EFFECT_SCHOOL_DAMAGE);
            }
        };

        SpellScript* GetSpellScript() const
        {
            return new spell_lei_shen_bouncing_bolt_dmg_SpellScript();
        }
};

class spell_area_lei_shen_crushing_thunder : public SpellAreaTriggerScript
{
    public:
        spell_area_lei_shen_crushing_thunder() : SpellAreaTriggerScript("spell_area_lei_shen_crushing_thunder") {}
        ...
        bool OnAddTarget(AreaTrigger* trigger, Unit* target)
        {
            ...
            trigger->GetCaster()->AddAura(SPELL_CRUSHING_THUNDER_PERIODIC_DMG, target);
...
class spell_area_lei_shen_lightning_whip : public SpellAreaTriggerScript
{
    public:
        spell_area_lei_shen_lightning_whip() : SpellAreaTriggerScript("spell_area_lei_shen_lightning_whip") {}
        ...
        bool OnAddTarget(AreaTrigger* trigger, Unit* target)
        {
            ...
            trigger->GetCaster()->AddAura(SPELL_LIGHTNING_BOLT, target);
        ...
        bool OnRemoveTarget(AreaTrigger* trigger, Unit* target)
        {
            target->RemoveAura(SPELL_LIGHTNING_BOLT);
    ...
    private:
        ...
        {
            if (!trigger->HasInArc(M_PI / 4, target))
            {
                return false;
            }
...
class spell_area_lei_shen_violent_gale_winds : public SpellAreaTriggerScript
{
    public:
        spell_area_lei_shen_violent_gale_winds() : SpellAreaTriggerScript("spell_area_lei_shen_violent_gale_winds") {}
        ...
        bool OnAddTarget(AreaTrigger* trigger, Unit* target)
        {
            ...
            trigger->GetCaster()->AddAura(SPELL_VIOLENT_GALE_WINDS_DMG, target);
...
void AddSC_boss_lei_shen()
{
    ...
    new spell_lei_shen_thunderstruck_dmg();         // 135096
    ...
    new spell_lei_shen_bouncing_bolt_dmg();         // 136366

    new spell_area_lei_shen_crushing_thunder();     // 135150
    new spell_area_lei_shen_lightning_whip();       // 137499
