    ...
    public:
        ...
        private:
            ...
            Creature* GetBoss(uint32 type)
            {
                switch (type)
                {
                    ...
                    case DATA_FROST_KING_MALAKK:
                        return sObjectAccessor->GetCreature(*me, instance->GetData64(type));
                }
                return NULL;
            }
            ...
            void InitBosses()
            {
                if (Creature* pKazrajin = GetBoss(DATA_KAZRAJIN))
                {
                    pKazrajin->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE);
                    if (pKazrajin->isDead())
                        pKazrajin->Respawn();
                }
                ...
                if (Creature* pSul = GetBoss(DATA_SUL_THE_SANDCRAWLER))
                {
                    pSul->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE);
                    if (pSul->isDead())
                        pSul->Respawn();
                }
                ...
                if (Creature* pMalakk = GetBoss(DATA_FROST_KING_MALAKK))
                {
                    pMalakk->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE);
                    if (pMalakk->isDead())
                        pMalakk->Respawn();
                }
                ...
                if (Creature* pMarli = GetBoss(DATA_HIGH_PRIESTESS_MARLI))
                {
                    pMarli->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE);
                    if (pMarli->isDead())
                        pMarli->Respawn();
                }
            }
            ...
            void StartBosses()
            {
                if (Creature* pKazrajin = GetBoss(DATA_KAZRAJIN))
                {
                    pKazrajin->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE);
                    pKazrajin->AI()->DoZoneInCombat(pKazrajin, 200.0f);
                }

                if (Creature* pSul = GetBoss(DATA_SUL_THE_SANDCRAWLER))
                {
                    pSul->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE);
                    pSul->AI()->DoZoneInCombat(pSul, 200.0f);
                }

                if (Creature* pMalakk = GetBoss(DATA_FROST_KING_MALAKK))
                {
                    pMalakk->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE);
                    pMalakk->AI()->DoZoneInCombat(pMalakk, 200.0f);
                }

                if (Creature* pMarli = GetBoss(DATA_HIGH_PRIESTESS_MARLI))
                {
                    pMarli->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE);
    ...
    public:
        ...
        struct boss_kazra_jinAI : public ScriptedAI
        {
            ...
            void JustDied(Unit* killer)
            {
                ...
                if (pInstance)
                {
                    if (Creature* Garajal = me->GetCreature(*me, pInstance->GetData64(DATA_GARAZHAL_THE_SPIRITBINDER)))
                    {
            ...
            void UpdateAI(uint32 const diff)
            {
                ...
                if (possesed)
                {
                    if (possesedDamagePct - me->GetHealthPct() >= 25.0f)
                    {
                        ...
                        if (Creature* pGarazhal = me->GetCreature(*me, pInstance->GetData64(DATA_GARAZHAL_THE_SPIRITBINDER)))
            ...
            Creature* GetBoss(uint32 type)
            {
                if (pInstance)
                {
                    switch (type)
                    {
                        ...
                        case DATA_HIGH_PRIESTESS_MARLI:
                        case DATA_FROST_KING_MALAKK:
                            return pInstance->instance->GetCreature(pInstance->GetData64(type));
                    }
                }
                return NULL;
            }
                    ...
                    switch (eventId)
                    {
                        ...
                        /*case EVENT_CHECK_TREACH_GROUND:
			            {
                            if (!despawnable)
                            {
                                ...
                                {
                                    if (CAST_AI(npc_sul_the_sandcrawler_quicksand::npc_sul_the_sandcrawler_quicksandAI, Quicksand->ToCreature()->AI())->despawnable)
                                    {
                                        if (Creature* boss = me->FindNearestCreature(NPC_SUL_THE_SANDCRAWLER, 200.0f, true))
    ...
    public:
        ...
        struct npc_blessed_loa_spiritAI : public ScriptedAI
        {
            ...
            void IsSummonedBy(Unit* summoner)
            {
                ...
                if (!councilorTarget)
                {
                    Creature* councilor = me->FindNearestCreature(RAND(NPC_KAZRAJIN, NPC_SUL_THE_SANDCRAWLER, NPC_FROST_KING_MALAKK), 200.0f, true);

                    if (!councilor)
                        councilor = me->FindNearestCreature(NPC_KAZRAJIN, 200.0f, true);

                    if (!councilor)
                        councilor = me->FindNearestCreature(NPC_SUL_THE_SANDCRAWLER, 200.0f, true);
        ...
        struct npc_shadowed_loa_spiritAI : public ScriptedAI
        {
            ...
            void MovementInform(uint32 type, uint32 id)
            {
                if (id == EVENT_CHARGE)
                {
                    ...
                    {
                        DoCast(player, SPELL_SHADOWED_GIFT_2, true);
    ...
    public:
        ...
        class spell_garajal_councilor_possessed_AuraScript : public AuraScript
        {
            ...
            void OnTick(AuraEffect const* aurEff)
            {
                ...
                if (target->ToCreature()->GetEntry() != NPC_KAZRAJIN && target->ToCreature()->GetEntry() != NPC_SUL_THE_SANDCRAWLER &&
                    target->ToCreature()->GetEntry() != NPC_FROST_KING_MALAKK && target->ToCreature()->GetEntry() != NPC_HIGH_PRIESTESS_MARLI)
                    return;
    ...
    public:
        ...
        class spell_malakk_frostbite_dmg_SpellScript : public SpellScript
        {
            ...
            void HandleAfterCast()
            {
                ...
                aur->SetStackAmount(newStackAmount);
            }
    ...
    public:
        ...
        class spell_kazrajin_reckless_charge_force_SpellScript : public SpellScript
        {
            ...
            void HandleHit(SpellEffIndex /*effIndex*/)
            {
                ...
                {
                    Position pos;
                    pKazrajin->GetNearPosition(pos, i, 0.0f);
    ...
    public:
        ...
        class spell_kazrajin_reckless_charge_dummy_SpellScript : public SpellScript
        {
            ...
            void HandleHit(SpellEffIndex /*effIndex*/)
            {
                ...
                {
                    Position pos;
                    GetCaster()->GetNearPosition(pos, i, 0.0f);
...
class spell_sul_the_sandcrawler_ensnared : public SpellScriptLoader
{
    public:
        spell_sul_the_sandcrawler_ensnared() : SpellScriptLoader("spell_sul_the_sandcrawler_ensnared") { }

        class spell_sul_the_sandcrawler_ensnared_AuraScript : public AuraScript
        {
            PrepareAuraScript(spell_sul_the_sandcrawler_ensnared_AuraScript);
            ...
            void OnApply(AuraEffect const* aurEff, AuraEffectHandleModes /*mode*/)
            {
                ...
                if (Aura const* aur = aurEff->GetBase())
                {
                    if (aur->GetStackAmount() >= 5)
                    {
                        GetCaster()->AddAura(SPELL_ENTRAPPED, GetUnitOwner());
                        Remove();
                    }
                }
            }
            ...
            void Register()
            {
                AfterEffectApply += AuraEffectApplyFn(spell_sul_the_sandcrawler_ensnared_AuraScript::OnApply, EFFECT_0, SPELL_AURA_MOD_DECREASE_SPEED, AURA_EFFECT_HANDLE_REAL_OR_REAPPLY_MASK);
            }
        };

        AuraScript* GetAuraScript() const
        {
            return new spell_sul_the_sandcrawler_ensnared_AuraScript();
        }
};

class spell_area_kazrajin_reckless_charge : public SpellAreaTriggerScript
{
    public:
        spell_area_kazrajin_reckless_charge() : SpellAreaTriggerScript("spell_area_kazrajin_reckless_charge") 
        {
        }
        ...
        bool OnAddTarget(AreaTrigger* trigger, Unit* target)
        {
            ...
            trigger->GetCaster()->CastSpell(target, SPELL_RECKLESS_CHARGE_DMG_2, true);
...
void AddSC_boss_council_of_elders()
{
    ...
    new spell_sul_the_sandcrawler_ensnared();

    new spell_area_kazrajin_reckless_charge();
