    ...
    public:
        ...
        bool OnGossipSelect(Player* p_Player, Creature* p_Creature, uint32 /*sender*/, uint32 p_Action)
        {
            ...
            if (p_Action == 1)
            {
                ...
                if (p_Player->GetQuestStatus(32805) == QUEST_STATUS_INCOMPLETE && p_Player->GetRoleConsideringSpe(p_Player->GetSpecializationId(p_Player->GetActiveSpec())) == Roles::ROLES_TANK)
                {
    ...
    public:
        ...
        struct npc_vision_of_deathwingAI : public ScriptedAI
        {
            ...
            void InitiateEvents()
            {
                ...
                DespawnCreaturesInArea(eDeathWingDatas::NpcMoltenElemental, me);
                DespawnCreaturesInArea(800001, me);
            }
            ...
            void UpdateAI(const uint32 p_Diff) override
            {
                ...
                switch (m_Events.ExecuteEvent())
                {
                    ...
                    case eDeathWingDatas::EventSummonAdds:
                    {
                        ...
                        DespawnCreaturesInArea(71041, me);
...
class spell_reign_of_fire_wrathion : public SpellScriptLoader
{
    public:
        spell_reign_of_fire_wrathion() : SpellScriptLoader("spell_reign_of_fire_wrathion") { }

        class spell_reign_of_fire_wrathion_AuraScript : public AuraScript
        {
            PrepareAuraScript(spell_reign_of_fire_wrathion_AuraScript);
            ...
            void OnTick(AuraEffect const* aurEff)
            {
                ...
                if (Unit* l_Unit = l_Owner->ToUnit())
                {
                    float l_Distance = frand(0.5f, 10.0f);
                    float l_Angle    = frand(0.0f, 6.14f);
                    float l_X = l_Unit->GetPositionX() + l_Distance * cos(l_Angle);
                    float l_Y = l_Unit->GetPositionY() + l_Distance * sin(l_Angle);

                    l_Unit->CastSpell(l_X, l_Y, l_Unit->m_positionZ, 141511, true);
                }
            }

            void Register()
            {
                OnEffectPeriodic += AuraEffectPeriodicFn(spell_reign_of_fire_wrathion_AuraScript::OnTick, EFFECT_0, SPELL_AURA_PERIODIC_DUMMY);
            }
        };

        AuraScript* GetAuraScript() const
        {
            return new spell_reign_of_fire_wrathion_AuraScript();
        }
};

/// White Tiger Temple Phases
class playerScript_enter_niuzao_temple : public PlayerScript
{
    public:
		playerScript_enter_niuzao_temple() : PlayerScript("playerScript_enter_niuzao_temple") { }
...
void AddSC_townlong_steppes()
{
    ...
    /// Misc
    new spell_reign_of_fire_wrathion();
    new playerScript_enter_niuzao_temple();
}