...
// 75657 - Energize
class spell_enegrize : public SpellScriptLoader
{
    public:
        spell_enegrize() : SpellScriptLoader("spell_enegrize_75657") { }

        class spell_enegrize_SpellScript : public SpellScript
        {
            PrepareSpellScript(spell_enegrize_SpellScript);

            void CheckTargets(WorldObject*& target)
            {
                if (!target || target->GetTypeId() != TYPEID_UNIT || target->ToUnit()->IsPetGuardianStuff())
                    target = nullptr;
            }

            void Register()
            {
                OnObjectTargetSelect += SpellObjectTargetSelectFn(spell_enegrize_SpellScript::CheckTargets, EFFECT_0, TARGET_UNIT_NEARBY_ENTRY);
            }
        };

        SpellScript* GetSpellScript() const
        {
            return new spell_enegrize_SpellScript();
        }
};
