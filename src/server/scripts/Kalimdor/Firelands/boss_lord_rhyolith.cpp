        ...
        struct boss_lord_rhyolithAI : public BossAI
        {
            ...
            void UpdateAI(const uint32 diff)
            {
                ...
                if (me->HealthBelowPct(25) && phase == 0)
                {
                    ...
                    Creature* LeftFoot  = Unit::GetCreature(*me, GuidLeftFoot);
                    Creature* RightFoot = Unit::GetCreature(*me, GuidRightFoot);
                    Creature* Controler = Unit::GetCreature(*me, GuidController);
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        {
                            if (Creature* Controler = Unit::GetCreature(*me, GuidController))
                            {
                                if (me->GetDistance2d(Controler) <= 0.5f)
                                {
                                    me->StopMoving();
                            ...
                            /*if (pController && (pLeftFoot && pLeftFoot->IsAIEnabled) && (pRightFoot && pRightFoot->IsAIEnabled))
                            {
                        ...
                        case EVENT_ACTIVATE_VOLCANO:
                        {
                            ...
                            me->GetCreatureListWithEntryInGrid(volcanos, NPC_VOLCANO, 300.0f); // TODO: adjuste searche range
