...
class boss_bethtilac : public CreatureScript
{
    public:
        ...
        struct boss_bethtilacAI : public BossAI
        {
            ...
            {
                _Reset();
                DespawnCreaturesInArea(NPC_CINDERWEB_SPINNER, me);
                DespawnCreaturesInArea(NPC_SPIDERWEB_FILAMENT, me);
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_CHECK_DRONE:
                            ...
                            {
                                if (Creature* pTarget = me->FindNearestCreature(NPC_CINDERWEB_DRONE, 100.0f))
                                {
                                    pDrone = pTarget;
