    ...
    struct npc_frightened_citizenAI : public ScriptedAI
    {
        ...
        WayPointID GetNearestPoint(Paths paths)
        {
            ...
            for (uint8 i = 0; i < PATHS_COUNT; i++)
            {
                ...
                for (uint8 j = 0; j < paths.pointsCount[i]; j++)
                {
                    if (j == 0)
                    {
    ...
    struct npc_bloodfang_worgenAI : public ScriptedAI
    {
        ...
        void UpdateAI(const uint32 diff)
        {
            if (tSeek <= diff)
            {
                ...
                {
                    Creature* enemy = NULL;
                    enemy = me->FindNearestCreature(NPC_SERGEANT_CLEESE, 10.0f, true);
                    if (!enemy)
                        enemy = me->FindNearestCreature(NPC_GILNEAN_ROYAL_GUARD, 10.0f, true);
