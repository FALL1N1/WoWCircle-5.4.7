...
public:
    ...
    static bool HandleAnticheatWarnCommand(ChatHandler* handler, const char* args)
    {
        ...
        if (command)
        {
            ...
            pTarget = sObjectAccessor->FindPlayerByNameInOrOutOfWorld(strCommand.c_str()); //get player by name
        }else
    ...
    static bool HandleAntiCheatDeleteCommand(ChatHandler* handler, const char* args)
    {
        ...
        {
            normalizePlayerName(strCommand);
            Player* player = sObjectAccessor->FindPlayerByNameInOrOutOfWorld(strCommand.c_str()); //get player by name
            if (!player)
    ...
    static bool HandleAntiCheatPlayerCommand(ChatHandler* handler, const char* args)
    {
        ...
        if (command)
        {
            ...
            normalizePlayerName(strCommand);
            player = sObjectAccessor->FindPlayerByNameInOrOutOfWorld(strCommand.c_str()); //get player by name
