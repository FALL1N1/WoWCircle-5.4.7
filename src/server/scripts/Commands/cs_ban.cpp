...
public:
    ...
    static bool HandleBanCharacterInterRealmCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
            return false;
        ...
        char* realmNameStr = strtok((char*)NULL, " ");
        if (!realmNameStr)
            return false;

        std::string realmName = realmNameStr;

        char* durationStr = strtok(NULL, " ");
        if (!durationStr || !atoi(durationStr))
            return false;

        std::string duration = durationStr;
        ...
        std::string gmName = handler->GetSession() ? handler->GetSession()->GetPlayer()->GetName() : "";

        if (InterRealmSession* session = sWorld->GetInterRealmSession())
            session->SendBanInterRealmCharacter(name, realmName, gmName, reason, duration);
