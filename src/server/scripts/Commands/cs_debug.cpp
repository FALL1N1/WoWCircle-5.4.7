    ...
    public:
        ...
        ChatCommand* GetCommands() const
        {
            ...
            static ChatCommand debugCommandTable[] =
            {
                ...
                { "df",             SEC_ADMINISTRATOR,  false, &HandleDebugDungeonFinderCommand,   "", NULL },
        ...
        static bool HandleDebugGetLootRecipientCommand(ChatHandler* handler, char const* /*args*/)
        {
            ...
            handler->PSendSysMessage("Loot recipient for creature %s (GUID %u, DB GUID %u) is %s", target->GetName(), target->GetGUIDLow(), target->GetSpawnId(), target->hasLootRecipient() ? (target->GetLootRecipient() ? target->GetLootRecipient()->GetName() : "offline") : "no loot recipient");
            return true;
        }
        ...
        static bool HandleDebugDungeonFinderCommand(ChatHandler* p_ChatHandler, char const*)
        {
            if (sLFGMgr->IsTest())
            {
                sLFGMgr->SetTesting(false);
                p_ChatHandler->SendSysMessage("Disable testing mode of dungeon finder");
            }
            else
            {
                sLFGMgr->SetTesting(true);
                p_ChatHandler->SendSysMessage("Enable testing mode of dungeon finder (2 players required in queue)");
            }
