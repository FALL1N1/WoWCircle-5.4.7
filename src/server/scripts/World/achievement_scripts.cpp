...
class AchievementCheck : public PlayerScript
{
    public:        
        AchievementCheck():PlayerScript("AchievementCheck") {}


        void OnLogin(Player* player)
        {
            BattlePet::PetJournal* journal = player->GetPetJournal();
            BattlePet::PetJournal::PetList const* pets = journal->GetPetList();

            for (BattlePet::PetJournal::PetList::const_iterator itr = pets->begin(); itr != pets->end(); ++itr)
            {
                uint32 spellId = 0;
                switch ((*itr)->GetSpecies())
                {
                    case 125:
                        spellId = 28738;
                        break;
                ...
                if (spellId)
                    player->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_LEARN_SPELL, spellId);
