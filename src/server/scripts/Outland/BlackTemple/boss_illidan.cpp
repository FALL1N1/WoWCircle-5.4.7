        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            for (uint8 i = 1; i <= MaxTimer; ++i)
                ...
                if (HealthBelowPct(50))
                {
                    ...
                    if (GETCRE(Illidan, IllidanGUID))
                    {
                        if (auto l_AI = CAST_AI(boss_illidan_stormrage::boss_illidan_stormrageAI, Illidan->AI()))
                            l_AI->DeleteFromThreatList(me->GetGUID());
                    }
                    me->AttackStop();
    ...
    case MAIEV_SHADOWSONG:
        {
            ...
            if (auto l_AI = CAST_AI(boss_maiev_shadowsong::boss_maievAI, summon->AI()))
                l_AI->GetIllidanGUID(me->GetGUID());
