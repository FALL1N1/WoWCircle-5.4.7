...
class spell_wicked_strong_fetish : public SpellScriptLoader
{
    ...
    enum Npcs
    {
        ...
        NPC_BLOOMAUL_EVIL_SPIRIT    = 21452,
    };
    ...
    public:
        ...
        class spell_wicked_strong_fetish_SpellScript : public SpellScript
        {
            ...
            void HandleScript(SpellEffIndex /*effIndex*/)
            {
                ...
                uint32 areaId = GetHitUnit()->GetAreaId();

                if (areaId == AREA_BLADESPIRE_OUTPOST || areaId == AREA_BLADESPIRE_HOLD)
                {
                    summonEntry = NPC_BLADESPIRE_EVIL_SPIRIT;
                }
                else if (areaId == AREA_BLOODMAUL_OUTPOST)
                {
                    summonEntry = NPC_BLOOMAUL_EVIL_SPIRIT;
                }

                if (summonEntry)
                {
                    GetHitUnit()->SummonCreature(summonEntry, *GetHitUnit(), TEMPSUMMON_TIMED_DESPAWN, 60000);
                }
            }
...
void AddSC_blades_edge_mountains()
{
    ...
    new spell_wicked_strong_fetish();       // 32580
}
