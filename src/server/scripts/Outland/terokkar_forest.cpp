...
public:
    ...
    struct mob_unkor_the_ruthlessAI : public ScriptedAI
    {
        ...
        void DamageTaken(Unit* done_by, uint32 &damage)
        {
            ...
            if (!player)
                player = done_by->GetCharmerOrOwnerPlayerOrPlayerItself();
            ...
            if (player && me->HealthBelowPctDamaged(30, damage))
            {
                if (Group* group = player->GetGroup())
                {
                    for (GroupReference* itr = group->GetFirstMember(); itr != NULL; itr = itr->next())
                    {
                        ...
                        if (member)
                        {
                            if (CompleteQuestForPlayer(member))
                            {
                                damage = 0;
                ...
                {
                    if (CompleteQuestForPlayer(player))
                    {
                        damage = 0;
                        isQuestCompleted = true;
                    }
    ...
    private:
        ...
        bool CompleteQuestForPlayer(Player* player)
        {
            if (player->GetQuestStatus(QUEST_DONTKILLTHEFATONE) == QUEST_STATUS_INCOMPLETE)
            {
