        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            if (Rage_Timer <= diff)
            {
                if (Creature* distiller = me->FindNearestCreature(17954, 100.0f))
                {
                    ...
                    if (distiller->IsAIEnabled)
                    {
                        if (mob_naga_distiller::mob_naga_distillerAI* l_NagaAI = CAST_AI(mob_naga_distiller::mob_naga_distillerAI, distiller->AI()))
                            l_NagaAI->StartRageGen(me);
                    }
                }
