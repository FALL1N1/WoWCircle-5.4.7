...
public:
    ...
    struct instance_eye_of_eternity_InstanceMapScript : public InstanceScript
    {
        ...
        void VortexHandling()
        {
            if (Creature* malygos = instance->GetCreature(malygosGUID))
            {
                for (uint64 vortexGuid : vortexTriggers)
                {
                    uint8 counter = 0;
                    if (Creature* trigger = instance->GetCreature(vortexGuid))
                    {
                        std::list<HostileReference*> m_threatlist = malygos->getThreatManager().getThreatList();

                        for (HostileReference* hostileRef : m_threatlist)
                        {
                            if (counter >= 5)
                                break;

                            if (Unit* target = hostileRef->getTarget())
                            {
