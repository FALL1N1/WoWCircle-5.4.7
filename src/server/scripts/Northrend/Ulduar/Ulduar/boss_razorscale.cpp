            ...
            void DoAction(int32 const action)
            {
                switch (action)
                {
                    ...
                    case ACTION_FIRE_OUT:
                        if (Creature* l_Engineer = Unit::GetCreature(*me, Engineer[0]))
                            l_Engineer->AI()->Talk(SAY_TURRETS);
                        break;
                }
            }
                ...
                if (AttackStartTimer <= Diff)
                {
                    switch (Phase)
                    {
                        ...
                        case 2:
                            for (uint8 n = 0; n < RAID_MODE(2, 4); n++)
                            {
                                Creature* l_Engineer = me->SummonCreature(NPC_ENGINEER, PosEngSpawn, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 3000);
                                Engineer[n] = l_Engineer ? l_Engineer->GetGUID() : 0;

                                if (l_Engineer)
                                {
                                    l_Engineer->SetWalk(false);
                                    l_Engineer->SetSpeed(MOVE_RUN, 0.5f);
                                    l_Engineer->SetHomePosition(PosEngRepair[n]);
                                    l_Engineer->GetMotionMaster()->MoveTargetedHome();

                                    if (n == 0)
                                        l_Engineer->AI()->Talk(SAY_READY);
                                }
                            }
                            Phase = 3;
                        ...
                        case 3:
                            for (uint8 n = 0; n < 4; n++)
                            {
                                Creature* l_Defender = me->SummonCreature(NPC_DEFENDER, PosDefSpawn[n], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 3000);
                                Defender[n] = l_Defender ? l_Defender->GetGUID() : 0;

                                if (l_Defender)
                                {
                                    l_Defender->SetWalk(false);
                                    l_Defender->SetHomePosition(PosDefCombat[n]);
                                    l_Defender->GetMotionMaster()->MoveTargetedHome();
                                }
                            }
                            Phase = 4;
                            break;
                        case 4:
                            for (uint8 n = 0; n < RAID_MODE(2, 4); n++)
                            {
                                if (Creature* l_Engineer = Unit::GetCreature(*me, Engineer[n]))
                                    l_Engineer->SetUInt32Value(UNIT_NPC_EMOTESTATE, EMOTE_STATE_USE_STANDING);
                            }
                            for (uint8 n = 0; n < 4; ++n)
                            {
                                if (Creature* l_Defender = Unit::GetCreature(*me, Defender[n]))
                                    l_Defender->SetUInt32Value(UNIT_NPC_EMOTESTATE, EMOTE_STATE_READY2H);
                            }
                        ...
                        case 5:
                            ...
                            if (Creature* l_Engineer = Unit::GetCreature(*me, Engineer[0]))
                                l_Engineer->AI()->Talk(SAY_PREPARED);
                            Phase = 6;
                            break;
                    }
                }
