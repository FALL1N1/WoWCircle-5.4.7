...
void CharacterDatabaseConnection::DoPrepareStatements()
{
    ...
    "health, power1, power2, power3, power4, power5, instance_id, speccount, activespec, specialization1, specialization2, exploredZones, equipmentCache, knownTitles, actionBars, currentpetslot, petslotused, grantableLevels, resetspecialization_cost, resetspecialization_time, bonusFactionId, bonusFactionLastTime FROM characters WHERE guid = ?", CONNECTION_ASYNC)
    ...
    PREPARE_STATEMENT(CHAR_SEL_CHARACTER_AURAS_EFFECTS, "SELECT slot, effect, baseamount, amount, data FROM character_aura_effect WHERE guid = ?", CONNECTION_ASYNC)
    ...
    "item, itemEntry, transEntry FROM character_inventory ci JOIN item_instance ii ON ci.item = ii.guid WHERE ci.guid = ? ORDER BY bag, slot", CONNECTION_ASYNC)
    ...
    PREPARE_STATEMENT(CHAR_SEL_OLD_CHARACTER_CRITERIAPROGRESS, "SELECT criteria, counter, date FROM character_achievement_progress WHERE guid = ?", CONNECTION_ASYNC)
    PREPARE_STATEMENT(CHAR_SEL_OLD_ACCOUNT_CRITERIAPROGRESS, "SELECT criteria, counter, date FROM account_achievement_progress WHERE account = ?", CONNECTION_ASYNC)
    PREPARE_STATEMENT(CHAR_SEL_CHARACTER_CRITERIAPROGRESS, "SELECT criteria, counter, date FROM character_achievement_criteria_progress WHERE guid = ?", CONNECTION_ASYNC)
    PREPARE_STATEMENT(CHAR_SEL_ACCOUNT_CRITERIAPROGRESS, "SELECT criteria, counter, date FROM account_achievement_criteria_progress WHERE account = ?", CONNECTION_ASYNC)
    ...
    PREPARE_STATEMENT(CHAR_SEL_CHARACTER_BGDATA, "SELECT instanceId, team, joinX, joinY, joinZ, joinO, joinMapId, taxiStart, taxiEnd, mountSpell, deserterTime, deserterCount FROM character_battleground_data WHERE guid = ?", CONNECTION_ASYNC)
    ...
    PREPARE_STATEMENT(CHAR_REP_ITEM_INSTANCE, "REPLACE INTO item_instance (itemEntry, transEntry, owner_guid, creatorGuid, giftCreatorGuid, count, duration, charges, flags, enchantments, randomPropertyId, reforgeId, transmogrifyId, upgradeId, durability, playedTime, text, guid) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC)
    PREPARE_STATEMENT(CHAR_UPD_ITEM_INSTANCE, "UPDATE item_instance SET itemEntry = ?, transEntry = ?, owner_guid = ?, creatorGuid = ?, giftCreatorGuid = ?, count = ?, duration = ?, charges = ?, flags = ?, enchantments = ?, randomPropertyId = ?, reforgeId = ?, transmogrifyId = ?, upgradeId = ?, durability = ?, playedTime = ?, text = ? WHERE guid = ?", CONNECTION_ASYNC)
    ...
    PREPARE_STATEMENT(CHAR_DEL_GUILD_ACHIEVEMENT_CRITERIA, "DELETE FROM guild_achievement_criteria_progress WHERE guildId = ? AND criteria = ?", CONNECTION_ASYNC)
    PREPARE_STATEMENT(CHAR_DEL_INVALID_ACHIEV_PROGRESS_CRITERIA_GUILD, "DELETE FROM guild_achievement_criteria_progress WHERE criteria = ?", CONNECTION_ASYNC);
    PREPARE_STATEMENT(CHAR_DEL_ALL_GUILD_ACHIEVEMENT_CRITERIA, "DELETE FROM guild_achievement_criteria_progress WHERE guildId = ?", CONNECTION_ASYNC)
    PREPARE_STATEMENT(CHAR_INS_GUILD_ACHIEVEMENT_CRITERIA, "INSERT INTO guild_achievement_criteria_progress (guildId, criteria, counter, date, completedGuid) VALUES (?, ?, ?, ?, ?)", CONNECTION_ASYNC)
    PREPARE_STATEMENT(CHAR_SEL_OLD_GUILD_ACHIEVEMENT_CRITERIA, "SELECT criteria, counter, date, completedGuid FROM guild_achievement_progress WHERE guildId = ?", CONNECTION_SYNCH)
    PREPARE_STATEMENT(CHAR_SEL_GUILD_ACHIEVEMENT_CRITERIA, "SELECT criteria, counter, date, completedGuid FROM guild_achievement_criteria_progress WHERE guildId = ?", CONNECTION_SYNCH)
    ...
    PREPARE_STATEMENT(CHAR_INS_AURA_EFFECT, "INSERT INTO character_aura_effect (guid, slot, effect, baseamount, amount, data) "
    "VALUES (?, ?, ?, ?, ?, ?)",  CONNECTION_ASYNC)
    ...
    // Rates
    PREPARE_STATEMENT(CHAR_SEL_RATES, "SELECT xpKill, xpQuest, repGain, repLowLvLKill, repLowLvLQuest, SkillGainCrafting, SkillGainGathering, SkillChanceOrange, SkillChanceYellow, SkillChanceGreen, SkillChanceGrey FROM character_rate WHERE guid = ?", CONNECTION_SYNCH);
    ...
    PREPARE_STATEMENT(CHAR_INS_PLAYER_BGDATA, "INSERT INTO character_battleground_data (guid, instanceId, team, joinX, joinY, joinZ, joinO, joinMapId, taxiStart, taxiEnd, mountSpell, deserterTime, deserterCount) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC)
    ...
    // Corpse
    PREPARE_STATEMENT(CHAR_SEL_CORPSES, "SELECT posX, posY, posZ, orientation, mapId, displayId, itemCache, bytes1, bytes2, flags, dynFlags, time, corpseType, instanceId, phaseMask, guid FROM corpse WHERE mapId = ? AND instanceId = ?", CONNECTION_SYNCH)
    PREPARE_STATEMENT(CHAR_INS_CORPSE, "INSERT INTO corpse (guid, posX, posY, posZ, orientation, mapId, displayId, itemCache, bytes1, bytes2, flags, dynFlags, time, corpseType, instanceId, phaseMask) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC)
    PREPARE_STATEMENT(CHAR_DEL_CORPSE, "DELETE FROM corpse WHERE guid = ?", CONNECTION_ASYNC);
    PREPARE_STATEMENT(CHAR_DEL_CORPSES_FROM_MAP, "DELETE FROM corpse WHERE mapId = ? AND instanceId = ?", CONNECTION_ASYNC);
    ...
    // Creature respawn for raid encounters
    PREPARE_STATEMENT(CHAR_SEL_CREATURE_RESPAWNS_FOR_RAID, "SELECT creatureGuid, respawnTime FROM creature_raid_respawn WHERE mapId = ? AND playerGuid = ?", CONNECTION_SYNCH)
    PREPARE_STATEMENT(CHAR_REP_CREATURE_RESPAWN_FOR_RAID, "REPLACE INTO creature_raid_respawn (playerGuid, respawnTime, creatureGuid, mapId) VALUES (?, ?, ?, ?)", CONNECTION_ASYNC)
    PREPARE_STATEMENT(CHAR_DEL_CREATURE_RESPAWN_FOR_RAID, "DELETE FROM creature_raid_respawn WHERE creatureGuid = ? AND mapId = ? AND playerGuid = ?", CONNECTION_ASYNC)
    PREPARE_STATEMENT(CHAR_DEL_CREATURE_RESPAWN_BY_ENCOUNTER, "DELETE FROM creature_raid_respawn WHERE mapId = ? AND playerGuid = ?", CONNECTION_ASYNC)
    ...
    // Gameobject respawn
    PREPARE_STATEMENT(CHAR_SEL_GO_RESPAWNS_FOR_RAID, "SELECT gameobjectGuid, respawnTime FROM gameobject_raid_respawn WHERE mapId = ? AND playerGuid = ?", CONNECTION_SYNCH)
    PREPARE_STATEMENT(CHAR_REP_GO_RESPAWN_FOR_RAID, "REPLACE INTO gameobject_raid_respawn (gameobjectGuid, respawnTime, mapId, playerGuid) VALUES (?, ?, ?, ?)", CONNECTION_ASYNC)
    PREPARE_STATEMENT(CHAR_DEL_GO_RESPAWN_FOR_RAID, "DELETE FROM gameobject_raid_respawn WHERE gameobjectGuid = ? AND mapId = ? AND playerGuid = ?", CONNECTION_ASYNC)
    PREPARE_STATEMENT(CHAR_DEL_GO_RESPAWN_BY_ENCOUNTER, "DELETE FROM gameobject_raid_respawn WHERE mapId = ? AND playerGuid = ?", CONNECTION_ASYNC)
    ...
    "power4, power5, latency, speccount, activespec, specialization1, specialization2, exploredZones, equipmentCache, knownTitles, actionBars, currentpetslot, petslotused, grantableLevels, bonusFactionId, bonusFactionLastTime) VALUES "
    "(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", CONNECTION_ASYNC);
    ...
    "equipmentCache=?,knownTitles=?,actionBars=?,currentpetslot=?,petslotused=?,grantableLevels=?,online=?, resetspecialization_cost = ?, resetspecialization_time = ?, bonusFactionId = ?, bonusFactionLastTime = ? WHERE guid=?", CONNECTION_ASYNC);
    ...
    PREPARE_STATEMENT(CHAR_DEL_OLD_INVALID_ACHIEV_PROGRESS_CRITERIA, "DELETE FROM character_achievement_progress WHERE criteria = ? AND guid = ?", CONNECTION_ASYNC);
    PREPARE_STATEMENT(ACC_DEL_OLD_INVALID_ACHIEV_PROGRESS_CRITERIA, "DELETE FROM account_achievement_progress WHERE criteria = ? AND account = ?", CONNECTION_ASYNC);
    PREPARE_STATEMENT(CHAR_DEL_INVALID_ACHIEV_PROGRESS_CRITERIA, "DELETE FROM character_achievement_criteria_progress WHERE criteria = ?", CONNECTION_ASYNC);
    PREPARE_STATEMENT(ACC_DEL_INVALID_ACHIEV_PROGRESS_CRITERIA, "DELETE FROM account_achievement_criteria_progress WHERE criteria = ?", CONNECTION_ASYNC);
    ...
    PREPARE_STATEMENT(CHAR_SEL_PET_AURA_EFFECT, "SELECT slot, effect, amount, baseamount, data FROM pet_aura_effect WHERE guid = ?",  CONNECTION_BOTH);
    ...
    PREPARE_STATEMENT(CHAR_INS_PET_AURA_EFFECT, "INSERT INTO pet_aura_effect (guid, slot, effect, baseamount, amount, data) VALUES (?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    ...
    PREPARE_STATEMENT(CHAR_DEL_CHAR_SKILL_LANGUAGES, "DELETE FROM character_skills WHERE skill IN (98, 113, 759, 111, 313, 109, 115, 315, 673, 137, 905, 906, 907, 791) AND guid = ?", CONNECTION_ASYNC);
    ...
    PREPARE_STATEMENT(CHAR_INIT_GUILD_CHALLENGES, "INSERT INTO guild_challenges VALUES (?, ?, 0)", CONNECTION_ASYNC);
    ...
    //////////////////////////////////////////////////////////////////////////
    /// Quest Currencies
    PREPARE_STATEMENT(CHAR_LOAD_QUEST_CURRENCY, "SELECT CurrencyId, CurrentCount, MaxCount FROM character_quest_currency WHERE CharacterGuid = ?", CONNECTION_SYNCH);
    PREPARE_STATEMENT(CHAR_INIT_QUEST_CURRENCY, "INSERT INTO character_quest_currency VALUE (?, ?, ?, ?)", CONNECTION_ASYNC);
    PREPARE_STATEMENT(CHAR_UPDATE_QUEST_CURRENCY, "UPDATE character_quest_currency SET CurrentCount = ? WHERE CharacterGuid = ? AND CurrencyId = ?", CONNECTION_ASYNC);
    PREPARE_STATEMENT(CHAR_RESET_QUEST_CURRENCY, "DELETE FROM character_quest_currency WHERE CharacterGuid = ? AND CurrencyId = ?", CONNECTION_ASYNC);
    //////////////////////////////////////////////////////////////////////////
